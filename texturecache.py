...
class MyConfiguration(object):
    def __init__(self, argv):
        ...
        self.DEBUG = (
            True if os.environ.get("PYTHONDEBUG", "n").lower() == "y" else False
        )
        ...
        if not config.has_section(self.THIS_SECTION):
            print(
                "Section [%s] is not a valid section in this config file"
                % self.THIS_SECTION
            )
            sys.exit(2)
        ...
        if UD_SYS_DEFAULT is None:
            UD_SYS_DEFAULT = self.getdefaultuserdata("XBMC")
        if UD_SYS_DEFAULT is None:
            UD_SYS_DEFAULT = "~/userdata"

        self.KODI_BASE = os.path.expanduser(
        ...
        self.PROFILE_ENCRYPTED = self.getBoolean(
            config, "profile.password.encrypted", "no"
        )
        ...
        if self.KODI_BASE[-1:] not in ["/", "\\"]:
            self.KODI_BASE += "/"
        if self.THUMBNAILS[-1:] not in ["/", "\\"]:
            self.THUMBNAILS += "/"
        ...
        self.WEB_CONNECTTIMEOUT = self.getValue(
            config, "webserver.ctimeout", 0.5, allowundefined=True
        )
        if self.WEB_CONNECTTIMEOUT:
            self.WEB_CONNECTTIMEOUT = float(self.WEB_CONNECTTIMEOUT)

        self.RPC_CONNECTTIMEOUT = self.getValue(
            config, "rpc.ctimeout", 0.5, allowundefined=True
        )
        if self.RPC_CONNECTTIMEOUT:
            self.RPC_CONNECTTIMEOUT = float(self.RPC_CONNECTTIMEOUT)
        ...
        self.QUERY_EPISODES = (
            self.getBoolean(config, "query.episodes", "yes")
            if self.QUERY_SEASONS
            else False
        )

        self.DOWNLOAD_THREADS_DEFAULT = int(
            self.getValue(config, "download.threads", "2")
        )
        ...
        self.DOWNLOAD_INTERVAL = float(
            self.getValue(config, "download.interval", "1.5")
        )
        ...
        for x in [
            ...
            temp = int(
                self.getValue(
                    config, "download.threads.%s" % x, self.DOWNLOAD_THREADS_DEFAULT
                )
            )
            self.DOWNLOAD_THREADS["download.threads.%s" % x] = temp

        self.SINGLETHREAD_URLS = self.getPatternFromList(
            config, "singlethread.urls", serial_urls, allowundefined=True
        )
        ...
        for x in [
            ...
            for f in ["zero", "blank", "art"]:
                ...
                if temp and temp.startswith("+"):
                    ...
                    self.QA_FIELDS[key] = (
                        temp if temp is not None else self.QA_FIELDS.get(key, None)
                    )
        ...
        self.QA_FAIL_MISSING_LOCAL_ART = self.getBoolean(
            config, "qa.fail.missinglocalart", "no"
        )
        self.QA_FAIL_TYPES = self.getPatternFromList(
            config, "qa.fail.urls", embedded_urls, allowundefined=True
        )
        self.QA_WARN_TYPES = self.getPatternFromList(config, "qa.warn.urls", "")

        (
            self.QA_NFO_REFRESH,
            self.qa_nfo_refresh_date,
            self.qa_nfo_refresh_date_fmt,
        ) = self.getRelativeDateAndFormat(config, "qa.nfo.refresh", "")
        self.QA_USEOLDREFRESHMETHOD = self.getBoolean(
            config, "qa.useoldrefreshmethod", "yes"
        )
        ...
        (
            self.CACHE_REFRESH,
            self.cache_refresh_date,
            self.cache_refresh_date_fmt,
        ) = self.getRelativeDateAndFormat(config, "cache.refresh", "")
        ...
        self.CACHE_EXTRA = (
            self.CACHE_EXTRA_FANART
            or self.CACHE_EXTRA_THUMBS
            or self.CACHE_VIDEO_EXTRAS
        )
        ...
        self.CACHE_IGNORE_TYPES = self.getPatternFromList(
            config, "cache.ignore.types", embedded_urls, allowundefined=True
        )
        self.PRUNE_RETAIN_TYPES = self.getPatternFromList(
            config, "prune.retain.types", ""
        )
        ...
        for index, r in enumerate(self.CACHE_IGNORE_TYPES):
            self.CACHE_IGNORE_TYPES[index] = re.compile(
                re.sub("^\\^image://", "^", r.pattern)
            )
        for index, r in enumerate(self.PRUNE_RETAIN_TYPES):
            self.PRUNE_RETAIN_TYPES[index] = re.compile(
                re.sub("^\\^image://", "^", r.pattern)
            )

        self.PRUNE_RETAIN_PREVIEWS = self.getBoolean(
            config, "prune.retain.previews", "yes"
        )
        self.PRUNE_RETAIN_PICTURES = self.getBoolean(
            config, "prune.retain.pictures", "no"
        )
        self.PRUNE_RETAIN_CHAPTERS = self.getBoolean(
            config, "prune.retain.chapters", "yes"
        )

        self.MISSING_IGNORE_PATTERNS = self.getPatternFromList(
            config, "missing.ignore.patterns", "", allowundefined=True
        )
        ...
        self.SUBTITLE_FILETYPES_EX = self.getFileExtList(
            config, "subtitle.filetypes", ""
        )
        ...
        self.IMDB_FIELDS_MOVIES = self.getExRepList(
            config, "imdb.fields.movies", ["rating", "votes", "top250"], True
        )
        self.IMDB_FIELDS_TVSHOWS = self.getExRepList(
            config, "imdb.fields.tvshows", ["rating", "votes"], True
        )
        self.IMDB_IGNORE_TVTITLES = self.getSimpleList(
            config, "imdb.ignore.tvtitles", "", True, "|"
        )
        self.IMDB_MAP_TVTITLES = self.getSimpleList(
            config, "imdb.map.tvtitles", "", True, "|"
        )
        self.IMDB_TRANSLATE_TVTITLES = self.getSimpleList(
            config, "imdb.translate.tvtitles", "", True, "|", False
        )
        self.IMDB_TRANSLATE_TVYEARS = self.getSimpleList(
            config, "imdb.translate.tvyears", "", True, "|"
        )
        self.IMDB_IGNORE_MISSING_EPISODES = self.getBoolean(
            config, "imdb.ignore.missing.episodes", "no"
        )
        ...
        self.IMDB_TIMEOUT = self.getValue(
            config, "imdb.timeout", 15.0, allowundefined=True
        )
        if self.IMDB_TIMEOUT:
            self.IMDB_TIMEOUT = float(self.IMDB_TIMEOUT)
        self.IMDB_RETRY = int(self.getValue(config, "imdb.retry", 3))
        self.IMDB_RETRY = 0 if self.IMDB_RETRY < 0 else self.IMDB_RETRY
        self.IMDB_GROUPING = self.getValue(
            config, "imdb.grouping", ",", allowundefined=True
        )
        self.IMDB_GROUPING = (
            self.IMDB_GROUPING if self.IMDB_GROUPING is not None else ""
        )
        self.IMDB_DEL_PARENTHESIS = self.getBoolean(
            config, "imdb.delete_parenthesis", "yes"
        )

        self.IMDB_PERIOD = self.getValue(
            config, "imdb.period", None, allowundefined=True
        )
        if self.IMDB_PERIOD:
            adate = datetime.date.today() - datetime.timedelta(
                days=int(self.IMDB_PERIOD)
            )
            self.IMDB_PERIOD_FROM = adate.strftime("%Y-%m-%d")
        else:
            self.IMDB_PERIOD_FROM = None

        self.BIN_TVSERVICE = self.getValue(
            config, "bin.tvservice", "/usr/bin/tvservice"
        )
        self.BIN_VCGENCMD = self.getValue(
            config, "bin.vcgencmd", "/usr/bin/vcgencmd", allowundefined=True
        )
    ...
    def getdefaultuserdata(self, appid):
        ...
        android1_path = "Android/data/org.%s.%s/files/.%s/userdata" % (
            appid.lower(),
            appid.lower(),
            appid.lower(),
        )
        ...
        if sys.platform == "win32":
            ...
            os.path.expanduser(macosx_path)
        ):
            return macosx_path
        else:  # Linux/Android
    ...
    def SetJSONVersion(self, major, minor, patch):
        ...
        self.JSON_HAS_OPEN_PLAYERCORE_DEFAULT = self.HasJSONCapability(
            "openplayercoredef"
        )
    ...
    def HasJSONCapability(self, feature):
        if feature not in self.JSON_VER_CAPABILITIES:
            raise ValueError(
                "Invalid JSON capability request for feature [%s]" % feature
            )
    ...
    def postConfig(self):
        ...
        if self.getValue(self.config, "download.predelete", "auto") == "auto":
            ...
            self.DOWNLOAD_PREDELETE = self.getBoolean(
                self.config, "download.predelete", "no"
            )
        ...
        self.MDATE_MDY = self.getBoolean(
            self.config, "modifieddate.mdy", "no"
    ...
    def getValue(self, config, aKey, default=None, allowundefined=False):
        ...
        try:
            ...
            if not value and allowundefined:
                return None
                ...
                try:
                    ...
                    if default is None and not allowundefined:
                        raise ConfigParser.NoOptionError(
                            aKey, "%s (or global section)" % self.THIS_SECTION
                        )
            else:
                if default is None and not allowundefined:
    ...
    def getSimpleList(
        self, config, aKey, default="", allowundefined=False, delimiter=",", strip=True
    ):
    ...
    def getFileExtList(self, config, aKey, default="", allowundefined=False):
        newList = []
        for x in [
            x.lower() for x in self.getSimpleList(config, aKey, default, allowundefined)
        ]:
            if not x.startswith("."):
                x = ".%s" % x
            if x not in newList:
    ...
    def getFilePath(self, filename=""):
        if os.path.isabs(self.THUMBNAILS):
            ...
            return os.path.join(
                self.CURRENT_PROFILE["tc.profilepath"], self.THUMBNAILS, filename
            )
    ...
    def showConfig(self):
        ...
        print(
            "  download.predelete = %s" % self.BooleanIsYesNo(self.DOWNLOAD_PREDELETE)
        )
        ...
        print(
            "  singlethread.urls = %s"
            % self.NoneIsBlank(self.getListFromPattern(self.SINGLETHREAD_URLS))
        )
        print(
            "  extrajson.addons  = %s"
            % self.NoneIsBlank(self.XTRAJSON["extrajson.addons"])
        )
        print(
            "  extrajson.agenres = %s"
            % self.NoneIsBlank(self.XTRAJSON["extrajson.agenres"])
        )
        print(
            "  extrajson.vgenres = %s"
            % self.NoneIsBlank(self.XTRAJSON["extrajson.vgenres"])
        )
        print(
            "  extrajson.albums  = %s"
            % self.NoneIsBlank(self.XTRAJSON["extrajson.albums"])
        )
        print(
            "  extrajson.artists = %s"
            % self.NoneIsBlank(self.XTRAJSON["extrajson.artists"])
        )
        print(
            "  extrajson.songs   = %s"
            % self.NoneIsBlank(self.XTRAJSON["extrajson.songs"])
        )
        print(
            "  extrajson.movies  = %s"
            % self.NoneIsBlank(self.XTRAJSON["extrajson.movies"])
        )
        print(
            "  extrajson.sets    = %s"
            % self.NoneIsBlank(self.XTRAJSON["extrajson.sets"])
        )
        print(
            "  extrajson.tvshows.tvshow = %s"
            % self.NoneIsBlank(self.XTRAJSON["extrajson.tvshows.tvshow"])
        )
        print(
            "  extrajson.tvshows.season = %s"
            % self.NoneIsBlank(self.XTRAJSON["extrajson.tvshows.season"])
        )
        print(
            "  extrajson.tvshows.episode= %s"
            % self.NoneIsBlank(self.XTRAJSON["extrajson.tvshows.episode"])
        )
        ...
        print(
            "  qa.nfo.refresh = %s%s"
            % (
                self.NoneIsBlank(self.QA_NFO_REFRESH),
                " (%s)" % self.qa_nfo_refresh_date_fmt
                if self.qa_nfo_refresh_date_fmt
                else "",
            )
        )
        print(
            "  qa.useoldrefreshmethod = %s"
            % (self.BooleanIsYesNo(self.QA_USEOLDREFRESHMETHOD))
        )
        print(
            "  qa.fail.checkexists = %s" % self.BooleanIsYesNo(self.QA_FAIL_CHECKEXISTS)
        )
        print(
            "  qa.fail.missinglocalart = %s"
            % self.BooleanIsYesNo(self.QA_FAIL_MISSING_LOCAL_ART)
        )
        print(
            "  qa.fail.urls = %s"
            % self.NoneIsBlank(self.getListFromPattern(self.QA_FAIL_TYPES))
        )
        print(
            "  qa.warn.urls = %s"
            % self.NoneIsBlank(self.getListFromPattern(self.QA_WARN_TYPES))
        )
        ...
        print(
            "  cache.hideallitems = %s" % self.BooleanIsYesNo(self.CACHE_HIDEALLITEMS)
        )
        print("  cache.artwork = %s" % self.NoneIsBlank(", ".join(self.CACHE_ARTWORK)))
        print(
            "  cache.ignore.types = %s"
            % self.NoneIsBlank(self.getListFromPattern(self.CACHE_IGNORE_TYPES))
        )
        ...
        print(
            "  cache.refresh = %s%s"
            % (
                self.NoneIsBlank(self.CACHE_REFRESH),
                " (%s)" % self.cache_refresh_date_fmt
                if self.cache_refresh_date_fmt
                else "",
            )
        )
        print("  cache.dropfile = %s" % self.NoneIsBlank(self.CACHE_DROP_INVALID_FILE))
        print(
            "  prune.retain.types = %s"
            % self.NoneIsBlank(self.getListFromPattern(self.PRUNE_RETAIN_TYPES))
        )
        print(
            "  prune.retain.previews = %s"
            % self.BooleanIsYesNo(self.PRUNE_RETAIN_PREVIEWS)
        )
        print(
            "  prune.retain.pictures = %s"
            % self.BooleanIsYesNo(self.PRUNE_RETAIN_PICTURES)
        )
        print(
            "  prune.retain.chapters = %s"
            % self.BooleanIsYesNo(self.PRUNE_RETAIN_CHAPTERS)
        )
        print(
            "  missing.ignore.patterns = %s"
            % self.NoneIsBlank(self.getListFromPattern(self.MISSING_IGNORE_PATTERNS))
        )
        ...
        temp = (
            " (%s)" % self.LASTRUNFILE_DATETIME
            if self.LASTRUNFILE and self.LASTRUNFILE_DATETIME
            else ""
        )
        print("  lastrunfile = %s%s" % (self.NoneIsBlank(self.LASTRUNFILE), temp))
        print(
            "  orphan.limit.check = %s" % self.BooleanIsYesNo(self.ORPHAN_LIMIT_CHECK)
        )
        print("  purge.minlen = %s" % self.PURGE_MIN_LEN)
        print(
            "  picture.filetypes = %s"
            % self.NoneIsBlank(", ".join(self.PICTURE_FILETYPES_EX))
        )
        print(
            "  video.filetypes = %s"
            % self.NoneIsBlank(", ".join(self.VIDEO_FILETYPES_EX))
        )
        print(
            "  audio.filetypes = %s"
            % self.NoneIsBlank(", ".join(self.AUDIO_FILETYPES_EX))
        )
        print(
            "  subtitle.filetypes = %s"
            % self.NoneIsBlank(", ".join(self.SUBTITLE_FILETYPES_EX))
        )
        print("  watched.overwrite = %s" % self.BooleanIsYesNo(self.WATCHEDOVERWRITE))
        print("  network.mac = %s" % self.NoneIsBlank(self.MAC_ADDRESS))
        print(
            "  imdb.fields.movies = %s"
            % self.NoneIsBlank(", ".join(self.IMDB_FIELDS_MOVIES))
        )
        print(
            "  imdb.fields.tvshows = %s"
            % self.NoneIsBlank(", ".join(self.IMDB_FIELDS_TVSHOWS))
        )
        print(
            "  imdb.ignore.tvtitles = %s"
            % self.NoneIsBlank("|".join(self.IMDB_IGNORE_TVTITLES))
        )
        print(
            "  imdb.map.tvtitles = %s"
            % self.NoneIsBlank("|".join(self.IMDB_MAP_TVTITLES))
        )
        print(
            "  imdb.translate.tvtitles = %s"
            % self.NoneIsBlank("|".join(self.IMDB_TRANSLATE_TVTITLES))
        )
        print(
            "  imdb.translate.tvyears = %s"
            % self.NoneIsBlank("|".join(self.IMDB_TRANSLATE_TVYEARS))
        )
        print(
            "  imdb.ignore.missing.episodes = %s"
            % self.BooleanIsYesNo(self.IMDB_IGNORE_MISSING_EPISODES)
        )
        ...
        if self.IMDB_PERIOD_FROM:
            print(
                "  imdb.period = %s (added after %s)"
                % (
                    self.NoneIsBlank(self.IMDB_PERIOD),
                    self.NoneIsBlank(self.IMDB_PERIOD_FROM),
                )
            )
        else:
            print("  imdb.period = %s" % (self.NoneIsBlank(self.IMDB_PERIOD)))
        print(
            "  imdb.delete_parenthesis = %s"
            % self.BooleanIsYesNo(self.IMDB_DEL_PARENTHESIS)
        )
        ...
        print(
            "  hdmi.force.hotplug = %s" % self.BooleanIsYesNo(self.HDMI_FORCE_HOTPLUG)
        )
        print(
            "  hdmi.ignoresuspend = %s" % self.BooleanIsYesNo(self.HDMI_IGNORE_SUSPEND)
        )
        print(
            "  hdmi.ignoredisable = %s" % self.BooleanIsYesNo(self.HDMI_IGNORE_DISABLE)
        )
        print("  hdmi.ignoreplayer = %s" % self.BooleanIsYesNo(self.HDMI_IGNORE_PLAYER))
        print(
            "  hdmi.ignorelibrary = %s" % self.BooleanIsYesNo(self.HDMI_IGNORE_LIBRARY)
        )
        ...
        print(
            "  profile.password.encrypted = %s"
            % self.BooleanIsYesNo(self.PROFILE_ENCRYPTED)
        )
    ...
    def debug(self, data, jsonrequest=None):
        with lock:
            if self.DEBUG:
                if self.ISATTY:
                    self.out(
                        "%s: [%s] %s" % (datetime.datetime.now(), self.OPTION, data),
                        newLine=True,
                    )
                else:
    ...
    def log(self, data, jsonrequest=None, maxLen=0):
        if self.LOGGING:
            with lock:
                ...
                if self.DEBUG or self.LOGFLUSH:
                    self.LOGFILE.flush()
    ...
    def log2(self, prefix, udata, jsonrequest=None, maxLen=0):
        if self.LOGGING:
            with lock:
                t = threading.current_thread().name
                self.LOGFILE.write(
                    "%s:%-10s: %s" % (datetime.datetime.now(), t, prefix)
                )
...
class MyImageLoader(threading.Thread):
    ...
    def geturl(self, item):
        ...
        if (
            url is None
            and not self.config.DOWNLOAD_PREDELETE
            and item.dbid != 0
            and self.force
        ):
            if self.config.HAS_THUMBNAILS_FS and not os.path.exists(
                self.config.getFilePath(item.cachedurl)
            ):
                self.logger.log(
                    "Deleting row with missing image from cache - id [%d], cachedurl [%s] for filename [%s]"
        ...
        while PDRETRY > 0 and url is None and isAvailable:
            self.logger.log(
                "Retrying getDownloadURL(), %d attempts remaining" % PDRETRY
            )
            # Introduce a short delay, unless we're playing back a log file...
            if self.json.LOG_REPLAYFILE is None:
                time.sleep(0.5)
            PDRETRY -= 1
            url = self.json.getDownloadURL(item.filename)
    ...
    def prime_the_request(self, url):
        ...
        try:
            ...
            self.logger.log(
                "Primed request of: HTTPS=%s, Domain [%s] with URL [%s], result [%d, %s]"
                % (
                    issecure,
                    domain,
                    page,
                    self.json.WEB_LAST_STATUS,
                    self.json.WEB_LAST_REASON,
                )
            )
            isAvailable = (
                self.json.WEB_LAST_STATUS == 200
                or 300 <= self.json.WEB_LAST_STATUS < 400
            )
        except Exception:
            isAvailable = False
    ...
    def loadImage(self, item):
        ...
        if url:
            if not self.config.DOWNLOAD_PREDELETE:
                if item.dbid != 0 and self.force:
                    if rowexists:
                        self.logger.log(
                            "Deleting old image from cache with id [%d], cachedurl [%s] for filename [%s]"
                            % (item.dbid, item.cachedurl, item.decoded_filename)
                        )
            ...
            self.logger.log(
                "Image not available for download - uncacheable (embedded?), or doesn't exist. Filename [%s]"
                % item.filename
            )
            ATTEMPT = 0
        ...
        while ATTEMPT > 0 and (self.config.DOWNLOAD_PAYLOAD or PERFORM_DOWNLOAD):
            try:
                ...
                PAYLOAD = self.json.sendWeb(
                    "GET", url, "loadImage", readAmount=1024, rawData=True
                )
                if self.json.WEB_LAST_STATUS == httplib.OK:
                    self.logger.log(
                        "Successfully downloaded image with size [%d] bytes, attempts required [%d], filename [%s]"
                        % (
                            len(PAYLOAD),
                            (self.retry - ATTEMPT + 1),
                            item.decoded_filename,
                        )
                    )
                    break
            except Exception:
            ...
            ATTEMPT -= 1
            self.logger.log(
                "Failed to download image URL [%s], status [%d], "
    ...
    def run(self):
        while not stopped.is_set():
            try:
                ...
                multipart_ep = qItem.get(
                    "multipart_ep", None
                )  # (season#, episode#) of first episode in a multipart episode
                ...
                if isMovie and self.movies250 is not None and imdbnumber is not None:
                    ...
                    needomdb = not (
                        movie250 != {}
                        and self.onlyt250fields
                        and (
                            "rank" in movie250
                            and "rating" in movie250
                            and "votes" in movie250
                        )
                    )
                else:
                    movie250 = {}
                ...
                if self.omdbquery and needomdb:
                    ...
                    while True:
                        if isMovie:
                            self.logger.log(
                                "Querying OMDb [a=%d, r=%d]: [movie] %s, imdb=%s"
                                % (attempt, self.retry, imdbnumber, title)
                            )
                            newimdb = (
                                MyUtility.getIMDBInfo(
                                    "movie",
                                    self.apikey,
                                    imdbnumber=imdbnumber,
                                    plotFull=self.plotFull,
                            ...
                            self.logger.log(
                                "Querying OMDb [a=%d, r=%d]: [tvshow] %s, %d, imdb=%s"
                                % (
                                    attempt,
                                    self.retry,
                                    title,
                            ...
                            self.logger.log(
                                "Querying OMDb [a=%d, r=%d]: [episode] %s, %d, S%02d, E%02d, imdb=%s"
                                % (
                                    attempt,
                                    self.retry,
                                    show_title,
                            ...
                            newimdb = MyUtility.getIMDBInfo(
                                ...
                                season=season,
                                episode=episode,
                                plotFull=(self.plotFull and not ismultipartquery),
                        ...
                        if newimdb is not None:
                            newimdb = (
                                newimdb
                                if newimdb.get("response", "False") == "True"
                                else None
                            )
                            ...
                            if (
                                newimdb is None
                                and multipart_ep is not None
                                and not ismultipartquery
                            ):
                                ismultipartquery = True
                                self.logger.log(
                                    "Re-querying OMDb [a=%d, r=%d]: [multipart] %s, %d, S%02d, E%02d => S%02d, E%02d"
                                    % (
                                        attempt,
                                        self.retry,
                                        show_title,
                    ...
                    if newimdb is None:
                        if isMovie:
                            logmsg = (
                                "Could not obtain OMDb details for [movie] %s (%s)"
                                % (imdbnumber, original_title)
                            )
                        elif isTVShow:
                            logmsg = (
                                "Could not obtain OMDb details for [tvshow] %s (%d)"
                                % (original_title, year)
                            )
                        elif isEpisode:
                            logmsg = (
                                "Could not obtain OMDb details for [episode] %s S%02dE%02d"
                    ...
                    self.logger.log(
                        "Avoided OMDb query, movies250 has all we need: %s (%s)"
                        % (imdbnumber, original_title)
                    )
                    newimdb = {}
                ...
                if self.movies250 is not None:
                    ...
                    newimdb["rating"] = movie250.get(
                        "rating", newimdb.get("rating", None)
                    )
...
class MyHDMIManager(threading.Thread):
    def __init__(self, config, logger, cmdqueue, hdmidelay=900, onstopdelay=5):
        ...
        self.bin_vcgencmd = (
            config.BIN_VCGENCMD
            if config.BIN_VCGENCMD and os.path.exists(config.BIN_VCGENCMD)
            else None
        )
        self.bin_ceccontrol = (
            config.BIN_CECCONTROL
            if config.BIN_CECCONTROL and os.path.exists(config.BIN_CECCONTROL)
            else None
        )
        ...
        self.logger.debug(
            "HDMI Power off delay: %d seconds (ignored when CanSuspend is yes)"
            % self.EventInterval(self.EV_HDMI_OFF)
        )
        self.logger.debug(
            "Player OnStop delay : %d seconds (ignored when CanSuspend is yes)"
            % self.EventInterval(self.EV_PLAY_STOP)
        )
        ...
        self.logger.debug(
            "Ignore Active Player: %s" % ("Yes" if self.ignoreplayer else "No")
        )
        self.logger.debug(
            "Ignore Library Scan : %s" % ("Yes" if self.ignorelibrary else "No")
        )
    ...
    def MonitorKodi(self):
        ...
        ordered_event_keys = [
            x[0] for x in sorted(self.events.items(), key=lambda e: e[1]["event.order"])
        ]
        ...
        while not stopped.is_set():
            try:
                ...
                if method == "pong":
                    ...
                    self.logger.debug(
                        "HDMI power management thread - initialising Kodi and HDMI state"
                    )
                    ...
                    library_active = (
                        clientState["scanning.music"] or clientState["scanning.video"]
                    )
                    ...
                    self.logger.debug(
                        "HDMI is [%s], Screensaver is [%s], Player is [%s], Library scan [%s], CanSuspend [%s], CanDisable [%s]"
                        % (
                            hdmi_status,
                ...
                elif method.endswith(".OnScanStarted") or method.endswith(
                    ".OnCleanStarted"
                ):
                    self.logger.debug("Library scan has started")
                    library_active = True

                elif method.endswith(".OnScanFinished") or method.endswith(
                    ".OnCleanFinished"
                ):
                ...
                for event in ordered_event_keys:
                    ...
                    if self.EventPending(event):
                        ...
                        if event == self.EV_HDMI_OFF:
                            self.logger.debug(
                                "HDMI power off in %d seconds unless cancelled"
                                % int(self.EventInterval(event))
                            )
                            if (player_active and not self.ignoreplayer) or (
                                library_active and not self.ignorelibrary
                            ):
                                self.logger.debug(
                                    "HDMI power-off will not occur until both player and library become inactive"
                                )
                        ...
                        if event == self.EV_PLAY_STOP:
                            ...
                            if (player_active and not self.ignoreplayer) or (
                                ...
                                if not self.EventOverdue(event, now):
                                    self.logger.debug(
                                        "HDMI power-off timeout reached - waiting for player and/or library to become inactive"
                                    )
                            else:
                    ...
                    if self.EventEnabled(event) and not self.EventOverdue(event, now):
                        ...
                        qtimeout = (
                            nextevent
                            if nextevent >= 0 and (not qtimeout or nextevent < qtimeout)
                            else qtimeout
                        )
    ...
    def sendKodiExit(self):
        ...
        MyJSONComms(self.config, self.logger).sendJSON(
            REQUEST, "libExit", checkResult=False
        )
    ...
    def getKodiStatus(self):
        ...
        REQUEST = {
            ...
            "params": {
                "properties": ["canshutdown", "cansuspend", "canreboot", "canhibernate"]
            },
        }
        ...
        statuses["cansuspend"] = (
            statuses["cansuspend"] and not self.config.HDMI_IGNORE_SUSPEND
        )
        ...
        if self.bin_vcgencmd:
            try:
                response = subprocess.check_output(
                    [self.bin_vcgencmd, "commands"], stderr=subprocess.STDOUT
                ).decode("utf-8")
                ...
                pass
        statuses["vcgencmd.display_power"] = (
            svalue and not self.config.HDMI_IGNORE_DISABLE
        )
    ...
    def getDisplayStatus(self):
        ...
        option = "--status"
        self.logger.log(
            "bin.tvservice (checking if TV is powered on) calling subprocess [%s %s]"
            % (self.bin_tvservice, option)
        )
        response = subprocess.check_output(
            [self.bin_tvservice, option], stderr=subprocess.STDOUT
        ).decode("utf-8")
        response = response[:-1] if response.endswith("\n") else response
    ...
    def getHDMIState(self):
        option = "--status"
        self.logger.log(
            "bin.tvservice (checking HDMI status) calling subprocess [%s %s]"
            % (self.bin_tvservice, option)
        )
        response = subprocess.check_output(
            [self.bin_tvservice, option], stderr=subprocess.STDOUT
    ...
    def setHDMIState(self, state):
        ...
        self.logger.log(
            "bin.tvservice (enabling/disabling HDMI) calling subprocess [%s %s]"
            % (self.bin_tvservice, option)
        )
        response = subprocess.check_output(
            [self.bin_tvservice, option], stderr=subprocess.STDOUT
    ...
    def getPowerState(self):
        self.logger.log(
            "bin.vcgencmd (checking HDMI power status) calling subprocess [%s %s]"
            % (self.bin_vcgencmd, "display_power")
        )
        response = subprocess.check_output(
            [self.bin_vcgencmd, "display_power"], stderr=subprocess.STDOUT
        ).decode("utf-8")
        response = response[:-1] if response.endswith("\n") else response
    ...
    def setPowerState(self, enabled):
        ...
        self.logger.log(
            "bin.vcgencmd (enabling/disabling HDMI power) calling subprocess [%s %s %s]"
            % (self.bin_vcgencmd, "display_power", option)
        )
        response = subprocess.check_output(
            [self.bin_vcgencmd, "display_power", option], stderr=subprocess.STDOUT
        ).decode("utf-8")
        response = response[:-1] if response.endswith("\n") else response
        self.logger.log("bin.vcgencmd response: [%s]" % response)
        return response.endswith("=1")
    ...
    def callCECControl(self, option):
        if self.bin_ceccontrol:
            ...
            self.logger.log(
                "bin.ceccontrol calling subprocess [%s %s]"
                % (self.bin_ceccontrol, option)
            )
            response = subprocess.check_output(
                [self.bin_ceccontrol, option], stderr=subprocess.STDOUT
            ).decode("utf-8")
            response = response[:-1] if response.endswith("\n") else response
    ...
    def disable_hdmi(self):
        if not self.getDisplayStatus():
            self.logger.debug(
                "Display device is not turned on, no need to disable HDMI"
            )
            return True
...
class MyDB(object):
        ...
        try:
            ...
            if str(e) == "database is locked":
                if self.RETRY < self.RETRY_MAX:
                    ...
                    self.RETRY += 1
                    self.logger.log(
                        "EXCEPTION SQL: %s - retrying attempt #%d" % (e, self.RETRY)
                    )
                    self.execute(SQL)
                else:
                    self.logger.out(
                        "ERROR: Database %s is locked - try again later."
    ...
    def _getAllColumns(self, filter, order):
        if self.DBVERSION >= 13:
            SQL = (
                "SELECT t.id, t.cachedurl, t.lasthashcheck, t.url, s.height, s.width, s.usecount, s.lastusetime, s.size, t.imagehash "
                "FROM texture t JOIN sizes s ON (t.id = s.idtexture)"
            )
        else:
            SQL = (
                "SELECT t.id, t.cachedurl, t.lasthashcheck, t.url, 0 as height, 0 as width, t.usecount, t.lastusetime, 0 as size, t.imagehash "
                "FROM texture t"
            )
    ...
    def deleteItem(self, id, cachedURL=None, warnmissing=True):
        ...
        if localFile is not None and os.path.exists(self.config.getFilePath(localFile)):
            ...
            self.logger.log(
                "FILE DELETE: Removed cached thumbnail file %s for id %s"
                % (localFile, (self.config.IDFORMAT % id))
            )
        else:
            if warnmissing:
                self.logger.out(
                    "WARNING: id %s, cached thumbnail file %s not found"
        ...
        if localFile:
            ...
            if localFile_dds and os.path.exists(self.config.getFilePath(localFile_dds)):
                ...
                self.logger.log(
                    "FILE DELETE: Removed cached thumbnail file %s for id %s"
                    % (localFile_dds, (self.config.IDFORMAT % id))
                )
    ...
    def getRowByFilename(self, filename):
        ...
        if not row:
            self.logger.log(
                "Failed to find row by filename with the expected formatting, trying again (with prefix, quoted)"
            )
    ...
    def dumpRow(self, row):
        line = "%s%s%-14s%s%04d%s%04d%s%04d%s%19s%s%19s%s%s\n" % (
            ...
            row["sizes"][0]["width"],
            self.config.FSEP,
            row["sizes"][0]["usecount"],
            self.config.FSEP,
            row["sizes"][0]["lastused"],
            self.config.FSEP,
            row["lasthashcheck"],
            self.config.FSEP,
            row["url"],
...
class MyJSONComms(object):
    ...
    def getSocket(self):
        ...
        if not self.mysocket:
            useipv = (
                int(self.config.RPC_IPVERSION) if self.config.RPC_IPVERSION else None
            )
            for ipversion in [socket.AF_INET6, socket.AF_INET]:
                if useipv and useipv == 4 and ipversion != socket.AF_INET:
                    continue
                if useipv and useipv == 6 and ipversion != socket.AF_INET6:
                    continue
                try:
                    ...
                    self.mysocket.connect(
                        (self.config.KODI_HOST, int(self.config.RPC_PORT))
                    )
                    self.mysocket.settimeout(None)
                    self.logger.log(
                        "RPC connection established with IPv%s"
                        % ("4" if ipversion == socket.AF_INET else "6")
                    )
                    self.config.RPC_IPVERSION = (
                        "4" if ipversion == socket.AF_INET else "6"
                    )
                ...
                raise lastexception if lastexception is not None else socket.error(
    ...
    def getWeb(self):
        if not self.myweb or self.config.WEB_SINGLESHOT:
            if self.myweb:
                self.myweb.close()
            self.myweb = httplib.HTTPConnection(
                "%s:%s" % (self.config.KODI_HOST, self.config.WEB_PORT),
                timeout=self.connecttimeout,
            )
            self.WEB_LAST_STATUS = -1
            self.WEB_LAST_REASON = ""
            if self.config.DEBUG:
                self.myweb.set_debuglevel(1)
        return self.myweb
    ...
    def logreplay_open(self):
        try:
            ...
            if thread not in self.config.log_replay_fmap:
                self.LOG_REPLAYFILE = codecs.open(
                    self.config.LOG_REPLAY_FILENAME, "r", encoding="utf-8"
                )
            ...
            self.logger.err(
                "ERROR: Unable to open replay file [%s] - exiting"
                % self.config.LOG_REPLAY_FILENAME,
                newLine=True,
                log=True,
            )
            sys.exit(2)
    ...
    def logreplay_mapthread(self, map_to_thread=None):
        ...
        self.web_re_result = re.compile(
            "^.*:(%s)[ ]*: .*\\.RECEIVED WEB DATA: ([0-9]*), (.*), (.*)$" % tpattern
        )
        self.json_re_result = re.compile(
            "^.*:(%s)[ ]*: .*\\.PARSING JSON DATA: (.*)$" % tpattern
        )
    ...
    def sendWeb(
        ...
        if request is not None:
            ...
            self.logger.log(
                "%s.JSON WEB REQUEST: [%s] [%s]" % (id, request_type, sdata)
            )
        else:
            sdata = None
            if domain is not None:
                self.logger.log(
                    "%s.DIRECT WEB REQUEST: [%s], HTTPS=%s, [%s] [%s]"
                    % (id, request_type, useSSL, domain, url)
                )
            else:
                self.logger.log(
                    "%s.DIRECT WEB REQUEST: [%s], [%s]" % (id, request_type, url)
                )
        ...
        if self.config.LOG_REPLAY_FILENAME:
            ...
            if domain:
                ...
                if self.config.WEB_AUTH_TOKEN:
                    headers.update(
                        {"Authorization": "Basic %s" % self.config.WEB_AUTH_TOKEN}
                    )
                web = self.getWeb()
            ...
            try:
                ...
                if self.WEB_LAST_STATUS == httplib.UNAUTHORIZED:
                    raise httplib.HTTPException(
                        "Remote web host requires webserver.username/webserver.password properties"
                    )
                ...
                if domain:
                    self.logger.log(
                        "%s.RECEIVED WEB DATA: %d, %s, <exception>"
                        % (id, response.status, response.reason),
                        maxLen=256,
                    )
                    raise
                if not self.config.WEB_SINGLESHOT:
        ...
        if self.logger.LOGGING:
            if rawData:
                self.logger.log(
                    "%s.RECEIVED WEB DATA: %d, %s, <raw data>"
                    % (id, self.WEB_LAST_STATUS, self.WEB_LAST_REASON),
                    maxLen=256,
                )
            else:
                self.logger.log(
                    "%s.RECEIVED WEB DATA: %d, %s, %s"
                    % (id, self.WEB_LAST_STATUS, self.WEB_LAST_REASON, data),
                    maxLen=256,
                )
    ...
    def sendJSON(
        ...
        if (
            request["method"] in ["Files.PrepareDownload", "Files.Download"]
            or useWebServer
        ):
            return self.sendWeb(
                "POST",
        ...
        while True:
            if ENDOFDATA:
                ...
                data = b""
                if jsocket:
                    jsocket.setblocking(1)

            try:
                if jsocket:
                    newdata = jsocket.recv(self.BUFFER_SIZE)
                    if len(data) == 0:
                        jsocket.settimeout(1.0)
                else:
                ...
                try:
                    ...
                    for m in self.parseResponse(udata):
                        if self.logger.LOGGING and messages == []:
                            if udata.find("\n") != -1:
                                self.logger.log2(
                                    "%s.PARSING JSON DATA: " % id,
                                    udata.replace("\t", "").replace("\n", ""),
                                    maxLen=256,
                                )
                            else:
                                self.logger.log2(
                                    "%s.PARSING JSON DATA: " % id, udata, maxLen=256
                                )
                        messages.append(m)
                    ...
                    self.logger.log(
                        "%s.PARSING COMPLETE, elapsed time: %f seconds"
                        % (id, time.time() - START_PARSE_TIME)
                    )
                    ...
                    if callback:
                        self.logger.log(
                            "%s.READING SOCKET UNTIL CALLBACK SUCCEEDS..." % id
                        )
                    else:
                    ...
                    if READ_ERR:
                        ...
                        self.logger.log(
                            "%s.Incomplete JSON data - continue reading socket" % id
                        )
                        if self.logger.VERBOSE:
                            self.logger.log2("Ignored Value Error: ", str(e))
                        continue
                except Exception as e:
        ...
        if cbjdata is not None:
            jdata = cbjdata
        if checkResult and "result" not in jdata:
            self.logger.out(
                "%s.ERROR: JSON response has no result!\n%s\n" % (id, jdata)
            )

        self.logger.log(
            "%s.FINISHED, elapsed time: %f seconds" % (id, time.time() - START_IO_TIME)
        )
        return jdata
    ...
    def handleResponse(self, callingId, jdata, callback):
        ...
        if callback:
            ...
            self.logger.log(
                "%s.PERFORMING CALLBACK: Name [%s], with Id [%s], Method [%s], Params [%s]"
                % (callingId, cname, id, method, params)
            )
            result = callback(id, method, params)
            self.logger.log(
                "%s.CALLBACK RESULT: [%s] Name [%s], Id [%s], Method [%s], Params [%s]"
                % (callingId, result, cname, id, method, params)
            )
            if result:
    ...
    def speak(self, id, method, params):
        ...
        if params["data"]:
            ...
            if item:
                title = self.getTitleForLibraryItem(
                    item.get("type", None), item.get("id", None)
                )
        ...
        if title:
            self.logger.out(
                "%s: %-21s: %s [%s]" % (datetime.datetime.now(), method, pmsg, title),
                newLine=True,
            )
        else:
            self.logger.out(
                "%s: %-21s: %s" % (datetime.datetime.now(), method, pmsg), newLine=True
            )
    ...
    def jsonWaitForScanFinished(self, id, method, params):
        if method.endswith("Library.OnUpdate") and "data" in params:
            ...
            if item:
                ...
                if title:
                    self.logger.out(
                        "Updating library: New %-9s %5d [%s]\n"
                        % (iType + "id", libraryId, title)
                    )
                else:
                    self.logger.out(
                        "Updating library: New %-9s %5d\n" % (iType + "id", libraryId)
                    )
    ...
    def rescanDirectories(self, workItems):
        ...
        doRefresh = (
            gConfig.JSON_HAS_REFRESH_REFACTOR and not gConfig.QA_USEOLDREFRESHMETHOD
        )
        ...
        for directory in sorted(workItems):
            ...
            for libraryitem in workItems[directory]:
                ...
                if doRefresh:
                    self.logger.log(
                        "Refreshing %s %d in media library." % (idName, libraryid)
                    )
                else:
                    self.logger.log(
                        "Removing %s %d from media library." % (idName, libraryid)
                    )
                ...
                if doRefresh:
                    gLogger.out(
                        "Updating library: Refreshed %s %d [%s]"
                        % (idName, libraryid, libraryitem["name"]),
                        newLine=True,
                    )

            if not doRefresh and not rootScan:
                self.scanDirectory(scanMethod, path=dpath)

        if not doRefresh and rootScan:
            self.scanDirectory(scanMethod)
    ...
    def scanDirectory(self, scanMethod, path=None):
        if path and path != "":
            ...
            self.logger.out(
                "Rescanning directory: %s..." % path, newLine=True, log=True
            )
        ...
        self.sendJSON(
            REQUEST,
            "libRescan",
            callback=self.jsonWaitForScanFinished,
            checkResult=False,
        )
    ...
    def cleanLibrary(self, cleanMethod, path=None):
        if path and path != "":
            self.logger.out(
                "Cleaning library directory: %s..." % path, newLine=True, log=True
            )
        ...
        self.sendJSON(
            REQUEST,
            "libClean",
            callback=self.jsonWaitForCleanFinished,
            checkResult=False,
        )
    ...
    def getDirectoryList(
        ...
        path,
        mediatype="files",
        properties=["file", "lastmodified"],
        use_cache=True,
        timestamp=False,
        honour_nomedia=False,
    ):
    ...
    def setTimeStamp(self, item):
        if "lastmodified" in item:
            try:
                item["lastmodified_timestamp"] = MyUtility.SinceEpoch(
                    datetime.datetime.strptime(
                        item["lastmodified"], self.config.qa_lastmodified_fmt
                    )
                )
            except ValueError:
                self.logger.err(
                    'ERROR: Invalid "lastmodified" date detected - try specifying @modifieddate.mdy=%s'
                    % ("no" if self.config.MDATE_MDY else "yes"),
                    newLine=True,
                )
                sys.exit(2)

    def getExtraArt(self, item):
        if not (item and self.config.CACHE_EXTRA):
            return []
            ...
            if "result" in data and "files" in data["result"]:
                for file in data["result"]["files"]:
                    if file["filetype"] == "file" and file["file"]:
                        if os.path.splitext(file["file"])[1].lower() in [
                            ".jpg",
                            ".png",
                            ".tbn",
                        ]:
                            files.append(
                                {
                                    "file": MyUtility.denormalise(
                                        file["file"], prefix=True
                                    ),
                                    "type": dir["type"].lower(),
                                }
                            )
    ...
    def getSeasonAll(self, filename):
        ...
        if self.config.CACHE_HIDEALLITEMS:
            return (None, None, None)

        # Not able to get a directory for remote files...
        if filename.find("image://http") != -1:
            return (None, None, None)
        ...
        if "result" in data and "files" in data["result"]:
            ...
            for f in data["result"]["files"]:
                if f["filetype"] == "file":
                    ...
                    if fname.startswith("season-all-poster."):
                        poster_url = MyUtility.joinQuotedPath(filename, f["file"])
                    elif not poster_url and fname.startswith("season-all."):
                        poster_url = MyUtility.joinQuotedPath(filename, f["file"])
                    elif fname.startswith("season-all-banner."):
                        banner_url = MyUtility.joinQuotedPath(filename, f["file"])
                    elif fname.startswith("season-all-fanart."):
                        fanart_url = MyUtility.joinQuotedPath(filename, f["file"])
            return (poster_url, fanart_url, banner_url)

        return (None, None, None)

    def getDownloadURL(self, filename):
        REQUEST = {"method": "Files.PrepareDownload", "params": {"path": filename}}
    ...
    def getFileDetails(self, filename, properties=["file", "lastmodified", "size"]):
        data = self.getDirectoryList(
            os.path.dirname(filename), mediatype="files", properties=properties
        )
        ...
        if "result" in data:
            ...
            for file in [
                x
                for x in files
                if x["filetype"] == "file" and x.get("file", None) == filename
            ]:
    ...
    def getSongName(self, songid):
        REQUEST = {
            ...
            "params": {
                "songid": songid,
                "properties": ["title", "artist", "albumartist"],
            },
        }
    ...
    def dumpJSON(self, data, decode=False, ensure_ascii=True):
        ...
        self.logger.out(
            json.dumps(data, indent=2, ensure_ascii=ensure_ascii, sort_keys=True),
            newLine=True,
        )
    ...
    def getAllFilesForSource(
        self, mediatype, labels, ignore_list=None, honour_nomedia=False
    ):
        if mediatype == "songs":
            mtype = "music"
        ...
        for label in labels:
            ...
            for path in sources:
                ...
                for file in self.getFiles(path, honour_nomedia):
                    ...
                    if ext not in includeList:
                        continue

                    if os.path.splitext(file)[0].lower().endswith("trailer"):
                        continue

                    isVIDEOTS = (
                        file.find("/VIDEO_TS/") != -1 or file.find("\\VIDEO_TS\\") != -1
                    )
                    isBDMV = file.find("/BDMV/") != -1 or file.find("\\BDMV\\") != -1

                    if isVIDEOTS and ext != ".vob":
                        continue
                    if isBDMV and ext != ".m2ts":
                        continue
    ...
    def getFilesForPath(self, fileList, path, honour_nomedia=False):
        data = self.getDirectoryList(
            path, use_cache=False, honour_nomedia=honour_nomedia
        )
        if "result" not in data:
            return
        if "files" not in data["result"]:
            ...
            if "file" in file:
                if file["filetype"] == "directory":
                    self.getFilesForPath(
                        fileList, os.path.dirname(file["file"]), honour_nomedia
                    )
                else:
    ...
    def getData(
        ...
        if mediatype == "addons":
            REQUEST = {
                ...
                "params": {
                    "properties": ["name", "version", "thumbnail", "fanart", "path"]
                },
            }
            FILTER = "name"
            TITLE = "name"
            ...
            REQUEST = {
                ...
                "params": {"type": subType, "properties": ["title", "thumbnail"]},
            }
            FILTER = "title"
            TITLE = "title"
            SECTION = "genres"
        ...
        if mediatype == "tags":
            if not filter or filter.strip() == "":
                self.addFilter(
                    REQUEST, {"field": "tag", "operator": "contains", "value": "%"}
                )
            else:
                word = 0
                filterBoolean = "and"
            ...
            if filter and not self.config.FILTER_FIELD:
                self.addFilter(
                    ...
                    {
                        "field": _f,
                        "operator": self.config.FILTER_OPERATOR,
                        "value": filter,
                    },
                )
            else:
                ...
                if self.config.JSON_HAS_FILTERNULLVALUE:
                    self.addFilter(
                        REQUEST, {"field": _f, "operator": "isnot", "value": ""}
                    )
                else:
                    self.addFilter(
                        REQUEST,
            ...
            if lastRun and self.config.LASTRUNFILE_DATETIME:
                self.addFilter(
                    REQUEST,
                    {
                        "field": "dateadded",
                        "operator": "after",
                        "value": self.config.LASTRUNFILE_DATETIME,
                    },
                )
        ...
        if action == "duplicates":
            ...
            if self.config.IMDB_PERIOD_FROM and mediatype in ["movies", "episodes"]:
                self.addFilter(
                    REQUEST,
                    {
                        "field": "dateadded",
                        "operator": "after",
                        "value": self.config.IMDB_PERIOD_FROM,
                    },
                )
            ...
            self.addProperties(
                REQUEST, ", ".join(self.config.getQAFields("zero", EXTRA))
            )
            self.addProperties(
                REQUEST, ", ".join(self.config.getQAFields("blank", EXTRA))
            )
            ...
            if "extrajson.%s" % EXTRA in self.config.XTRAJSON:
                extraFields = (
                    self.config.XTRAJSON["extrajson.%s" % EXTRA]
                    if EXTRA != ""
                    else None
                )
            else:
        ...
        elif action == "query" and mediatype not in [
            "tvshows",
            "seasons",
            "pvr.tv",
            "pvr.radio",
            ...
            if (
                mediatype in ["movies", "tags", "tvshows", "episodes"]
                and self.config.CACHE_CAST_THUMB
            ):
    ...
    def getDataProxy(
        self, mediatype, request, trim_cast_thumbs=True, idname=None, uniquecast=None
    ):
        if not idname:
        ...
        if self.config.CHUNKED:
            ...
            if (
                "result" in data
                and trim_cast_thumbs
                and "cast" in request.get("params", {}).get("properties", [])
            ):
    ...
    def chunkedLoad(
        ...
        while chunk_start < total_items:
            ...
            if not silent:
                ...
                if chunk_start != 0:
                    self.logger.progress(
                        "Loading %s: Chunk %d of %d..."
                        % (mediatype.capitalize(), chunk, chunks)
                    )
                else:
                    self.logger.progress(
                        "Loading %s: Chunk %d..." % (mediatype.capitalize(), chunk)
                    )
            ...
            if chunk_start == 0:
                ...
                self.logger.log(
                    "Chunk processing: found %d %s, retrieving in chunks of %d"
                    % (total_items, mediatype, CHUNK_SIZE)
                )
            ...
            if section and section in data["result"]:
                ...
                if trim_cast_thumbs and "cast" in request.get("params", {}).get(
                    "properties", []
                ):
                    for item in data["result"][section]:
                        self.removecastwithoutthumbs(item, uniquecast)
        ...
        response = {
            "result": {
                "limits": {"start": 0, "end": len(results), "total": len(results)}
            }
        }
        if section:
            response["result"][section] = results
        return response
        ...
        for file in data["result"]["files"]:
            if file["file"]:
                ...
                if ftype == "directory":
                    ...
                    in self.config.picture_filetypes
                ):
                    if addPreviews:
                        if not DIR_ADDED:
                            DIR_ADDED = True
                            list.append(
                                {
                                    "type": "directory",
                                    "label": path,
                                    "thumbnail": "picturefolder@%s" % path,
                                }
                            )
                        list.append(
                            {
                                "type": "file",
                                "label": fname,
                                "thumbnail": "%s/transform?size=thumb" % fname,
                            }
                        )
                    if addPictures:
                        list.append(
                            {"type": "file", "label": fname, "thumbnail": fname}
                        )
        ...
        for token in PATTERN.split(filter)[1::2]:
            ...
            if f == 3:
                ...
                if fields[1].endswith("thanequal"):
                    ...
                    data.append(
                        {"field": fields[0], "operator": fields[1], "value": fields[2]}
                    )
                    if group:
                        if stack == []:
    ...
    def getTextures(self, filter=None, order=None, allfields=False):
        ...
        if allfields:
            REQUEST["params"]["properties"].extend(
                ["lasthashcheck", "imagehash", "sizes"]
            )
    ...
    def delTexture(self, id):
        REQUEST = {"method": "Textures.RemoveTexture", "params": {"textureid": id}}
...
class MyTotals(object):
    ...
    def addSeasonAll(self):
        if "Season-all" not in self.TOTALS:
            self.TOTALS["Season-all"] = {}

    def addNotCached(self):
        if "Not in Cache" not in self.TOTALS:
            self.TOTALS["Not in Cache"] = {}

    def TimeStart(self, mediatype, item):
        if mediatype not in self.TIMES:
            self.TIMES[mediatype] = {}
    ...
    def start(self, mediatype, imgtype):
        with lock:
            ...
            if mediatype not in self.ETIMES:
                self.ETIMES[mediatype] = {}
            if imgtype not in self.ETIMES[mediatype]:
                self.ETIMES[mediatype][imgtype] = {}
            if tname not in self.ETIMES[mediatype][imgtype]:
                self.ETIMES[mediatype][imgtype][tname] = (ctime, 0)
    ...
    def finish(self, mediatype, imgtype):
        with lock:
            ...
            self.THREADS[tname] = 0
            self.ETIMES[mediatype][imgtype][tname] = (
                self.ETIMES[mediatype][imgtype][tname][0],
                ctime,
            )
    ...
    def bump(self, action, imgtype):
        with lock:
            if action not in self.TOTALS:
                self.TOTALS[action] = {}
            if imgtype not in self.TOTALS[action]:
                self.TOTALS[action][imgtype] = 0
    ...
    def setPerformance(self, elapsed):
        with lock:
            ...
            self.PCOUNT += 1
            self.PAVG += elapsed
            if self.PMIN == 0 or elapsed < self.PMIN:
                self.PMIN = elapsed
            if elapsed > self.PMAX:
                self.PMAX = elapsed
    ...
    def libraryStats(self, item="", multi=[], filter="", lastRun=False, query=""):
        ...
        if item.find("pvr.") != -1:
            ...
            if "fanart" not in items:
                items["fanart"] = None
        if item.find("movies") != -1:
            if "poster" not in items:
                items["poster"] = None
        if item.find("tvshows") != -1:
            if "thumb" not in items:
                items["thumb"] = None
        if (
            item.find("artists") != -1
            or item.find("albums") != -1
            or item.find("songs") != -1
        ):
            if "thumbnail" not in items:
                items["thumbnail"] = None
                ...
                for itype in self.ETIMES[mtype]:
                    ...
                    for tname in self.ETIMES[mtype][itype]:
                        ...
                        if tname.startswith("Main"):
                            ...
                            if tuple[0] < itmin or itmin == 0.0:
                                itmin = tuple[0]
                            if tuple[1] > itmax:
                                itmax = tuple[1]
                            if itype not in self.TOTALS[DOWNLOAD_LABEL]:
                                self.TOTALS[DOWNLOAD_LABEL][itype] = 0
                    self.TOTALS[DOWNLOAD_LABEL][itype] = (itmax - itmin) + (
                        mtmax - mtmin
                    )
                    if itmin < tmin or tmin == 0.0:
                        tmin = itmin
                    if itmax > tmax:
                        tmax = itmax
                    if mtmin < mmin or mmin == 0.0:
                        mmin = mtmin
                    if mtmax > mmax:
                        mmax = mtmax
        ...
        if filter != "":
            line0 = '%s, filtered by "%s"' % (line0, filter)
        if lastRun and self.LASTRUNDATETIME:
    ...
    def libraryStatsSummary(self):
        ...
        if not self.gotTimeDuration("Load"):
            return
        ...
        if len(self.THREADS_HIST) != 0:
            ...
            print(
                "   Min/Avg/Max: %05.2f / %05.2f / %05.2f downloads per second"
                % (self.MMIN, self.MCOUNT / mavg, self.MMAX)
            )
            print(
                "   Min/Avg/Max: %05.2f / %05.2f / %05.2f seconds per download"
                % (self.PMIN, self.PAVG / pcount, self.PMAX)
            )
            print("")
        ...
        if self.gotTimeDuration("Compare"):
            print(
                "     Comparing: %s" % self.secondsToTime(self.TimeDuration("Compare"))
            )
        if self.gotTimeDuration("Rescan"):
            print(
                "    Rescanning: %s" % self.secondsToTime(self.TimeDuration("Rescan"))
            )

        if self.gotTimeDuration("PreDelete"):
            print(
                "  Pre-Deleting: %s"
                % self.secondsToTime(self.TimeDuration("PreDelete"))
            )

        if len(self.THREADS_HIST) != 0:
            print(
                "   Downloading: %s" % self.secondsToTime(self.TimeDuration("Download"))
            )
...
class MyMediaItem(object):
    ...
    def __init__(
        ...
        self.decoded_filename = (
            MyUtility.normalise(self.filename, strip=True)
            if self.filename
            else self.filename
        )
        self.dbid = dbid
...
class MyWatchedItem(object):
    ...
    def match(self, mediatype, name, episode_year):
        if mediatype != self.mtype:
            return False

        xepisode_year = episode_year
        if xepisode_year is None:
            xepisode_year = ""
    ...
    def refresh(self, HAS_RESUME, playcount, lastplayed, resume):
        ...
        if playcount > self.playcount:
            self.playcount = playcount
        if lastplayed > self.lastplayed:
            self.lastplayed = lastplayed
    ...
    def setState(self, HAS_RESUME, playcount, lastplayed, resume):
        ...
        if self.playcount == playcount and self.lastplayed == lastplayed:
            if not HAS_RESUME:
                return
            if self.resume["position"] == resume["position"]:
                return
...
class MyUtility(object):
    ...
    RE_STACKING_1_9 = re.compile(
        "(.*?)([ _.-]*(?:cd|dvd|p(?:ar)?t|dis[ck])[ _.-]*[0-9]+)(.*?)(\\.[^.]+)$",
        flags=re.IGNORECASE,
    )
    RE_STACKING_A_D = re.compile(
    ...
    def toUnicode(data):
        ...
        if isinstance(data, basestring):
            if not isinstance(data, unicode):  # noqa: F821
                try:
                    data = unicode(data, encoding="utf-8", errors="ignore")  # noqa: F821
                except UnicodeDecodeError:
                    pass
    ...
    def Top250MovieList():
        ...
        try:
            ...
            if tstamp != -1:
                tstamp += 30
                gLogger.log(
                    "Top250: Data last updated [%s]" % data[tstamp : tstamp + 18]
                )
            ...
            if spos != -1 and epos != -1:
                ...
                movies = {}
                for row in table:
                    if row.attrib["class"] == "row_header":
                        continue
                    movie = {}
                    ...
                    if s:
                        ...
                        try:
                            if gConfig.JSON_VOTES_HAVE_NO_GROUPING:
                                movie["votes"] = "%s" % int(
                                    MyUtility.getDigits(row[4].text)
                                )
                            else:
                                movie["votes"] = "%s" % format(
                                    int(MyUtility.getDigits(row[4].text)), ",d"
                                ).replace(",", gConfig.IMDB_GROUPING)
                        except Exception:
                            pass
                ...
                gLogger.log(
                    "Top250: ERROR: didn't find movie data, skipping Top250 movies"
                )
                return None
        except Exception as e:
            gLogger.log(
                "Top250: ERROR: failed to retrieve Top250 movie data: [%s]" % str(e)
            )
            raise
            return None
        ...
        try:
            ...
            if mediatype == "movie":
                ...
                if imdbnumber is not None:
                    ...
                    query_url = "t=%s&y=%d" % (
                        MyUtility.denormalise(title, False),
                        year,
                    )
                isTVShow = True
                if season:
                ...
                if episode:
                    ...
                    reference = "episode: %s (%d) S%02dE%02d" % (
                        title,
                        year,
                        season,
                        episode,
                ...
                query_url = "%s&type=%s" % (
                    query_url,
                    "episode" if isEpisode else "series",
                )
            else:
                return {}
            ...
            if isMovie:
                f = urllib2.urlopen(
                    "%s&%s&plot=short" % (base_url, query_url), timeout=qtimeout
                )
            ...
            if not isMovie or plotFull:
                f = urllib2.urlopen(
                    "%s&%s&plot=full" % (base_url, query_url), timeout=qtimeout
                )
            ...
            if "Response" not in data or data["Response"] == "False":
                gLogger.log(
                    "Failed OMDb API Query [%s&%s] => [%s]"
                    % (redacted_url, query_url, data)
                )
                if isTVShow and not isEpisode:
                    gLogger.log(
                        "Try OMDb API Query [%s&s=%s&type=series] to see possible available titles (hint: year of tvshow is %d)"
                        % (redacted_url, title, year)
                    )
                return {}
                ...
                try:
                    ...
                    # Runtime from "2 h", "36 min", "2 h 22 min" or "N/A" to seconds
                        ...
                        if gConfig.JSON_VOTES_HAVE_NO_GROUPING:
                            ...
                            newdata[newkey] = format(
                                int(MyUtility.getDigits(data[key])), ",d"
                            ).replace(",", gConfig.IMDB_GROUPING)
                    elif newkey == "rated":
                        newkey = "mpaa"
                        if data[key] is None or data[key] in ["Not Rated", "Unrated"]:
                            newdata[newkey] = "Not Rated"
                        else:
                            newdata[newkey] = "Rated %s" % data[key]
                    elif newkey == "released":
                        newkey = "premiered"
                        newdata[newkey] = datetime.datetime.strptime(
                            data[key].replace(" ", "-"), "%d-%b-%Y"
                        ).strftime("%Y-%m-%d")
                    else:
                    ...
                    gLogger.log(
                        "Exception during IMDb processing: reference [%s], key [%s]. msg [%s]"
                        % (reference, key, str(e))
                    )
            ...
            gLogger.log(
                "Exception during IMDb processing: reference [%s], timeout [%s], msg [%s]"
                % (reference, qtimeout, str(e))
            )
            gLogger.log("OMDb API Query [%s&%s]" % (redacted_url, query_url))
            return {}
    ...
    def is_cache_item_stale(config, jcomms, mediaitem):
        if config.cache_refresh_date is None:
            return False
        if mediaitem.decoded_filename.startswith("http://"):
            return False

        # Only check file details for the following media types
        if mediaitem.mtype in [
            "movies",
            "tags",
            "sets",
            "tvshows",
            ...
            file = jcomms.getFileDetails(
                mediaitem.decoded_filename, properties=["file", "lastmodified"]
            )
        ...
        with lock:
            ...
            for p in MyUtility.DCData:
                for c in MyUtility.DCStats[p]:
                    if c not in MyUtility.DCStatsAccumulated:
                        MyUtility.DCStatsAccumulated[c] = 0
    ...
    def setDirectoryCacheItem(data, properties, path):
        ...
        with lock:
            ...
            if gConfig.LOGDCACHE:
                ...
                gLogger.log(
                    "Directory Cache %4s: %s (%s) [hit #1. %d items in cache]"
    ...
    def getDirectoryCacheItem(properties, path):
        ...
        with lock:
            ...
            if gConfig.LOGDCACHE:
                ...
                size = len(MyUtility.DCData[props])
                gLogger.log(
                    "Directory Cache %4s: %s (%s) [hit #%d, %d items in cache]"
    ...
    def trimDirectoryCache(properties):
        if properties not in MyUtility.DCData:
            return
        if len(MyUtility.DCData[properties]) <= gConfig.DCACHE_SIZE:
            return
        ...
        for ditem in delKeys:
            if gConfig.LOGDCACHE:
                gLogger.log(
                    "Directory Cache TRIM: %s (%s) [%d hits, %d items] (Age: %5.2f seconds)"
                    % (
                        properties,
                        ditem,
                        MyUtility.DCData[properties][ditem]["count"],
                        len(MyUtility.DCData[properties]),
                        (now - cp[ditem]["time"]),
                    )
                )
            del cp[ditem]
        ...
        while len(cp) > gConfig.DCACHE_SIZE:
            ...
            if oldestItem:
                ...
                if gConfig.LOGDCACHE:
                    gLogger.log(
                        "Directory Cache TRIM: %s (%s) [%d hits, %d items] (Size)"
    ...
    def logDirectoryCacheStats(mediatype=None, totals=False):
        if gLogger.LOGGING:
            if totals and MyUtility.DCStatsAccumulated:
                gLogger.log(
                    "Directory Cache Config: Maximum Size %d, Age Limit %d seconds"
                    % (gConfig.DCACHE_SIZE, gConfig.DCACHE_AGELIMIT)
                )
                stats = MyUtility.DCStatsAccumulated
                gLogger.log(
                    "Directory Cache Totals: Misses %d, Stores %d, Hits %d, Evicted %d"
                ...
                for props in MyUtility.DCStats:
                    ...
                    gLogger.log(
                        "Directory Cache PERF: Misses %d, Stores %d, Hits %d, Evicted %d, mediatype [%s] for properties: %s"
                        % (
                            stats["miss"],
                            stats["store"],
                            stats["hit"],
...
def jsonQuery(
    ...
    if action == "missing" and mediatype not in [
        "songs",
        "movies",
        "tvshows",
        "seasons",
        ...
        gLogger.err(
            "ERROR: media class [%s] is not currently supported by missing" % mediatype,
            newLine=True,
        )
        sys.exit(2)
    ...
    if action == "watched" and mediatype not in ["movies", "tvshows"]:
        gLogger.err(
            "ERROR: media class [%s] is not currently supported by watched" % mediatype,
            newLine=True,
        )
        sys.exit(2)
    ...
    if action == "imdb" and mediatype not in ["movies", "tvshows"]:
        gLogger.err(
            "ERROR: media class [%s] is not currently supported by IMDb" % mediatype,
            newLine=True,
        )
        return
    ...
    if mediatype in ["pvr.tv", "pvr.radio"] and not gConfig.HAS_PVR:
        ...
        for subtype in ["movie", "tvshow", "musicvideo"]:
            (section_name, title_name, id_name, data) = jcomms.getData(
                action,
                mediatype,
                filter,
                extraFields,
            ...
            if data and "result" in data and section_name in data["result"]:
                ...
                if len(data["result"][section_name]) > 0:
                    _data.append(
                        {"type": subtype, section_name: data["result"][section_name]}
                    )
        title_name = "type"
    ...
    if (
        data
        and filter
        and mediatype in ["addons", "agenres", "sets", "pvr.tv", "pvr.radio"]
    ):
        filter_name = gConfig.FILTER_FIELD if gConfig.FILTER_FIELD else title_name
    ...
    if mediatype == "tvshows" and gConfig.QUERY_SEASONS:
        for tvshow in data:
            ...
            for season in tvshow[s2]:
                ...
                if seasonid < 0:
                    gLogger.err(
                        "WARNING: TV show [%s] has invalid season (%d) - ignored"
                        % (title, seasonid),
                        newLine=True,
                    )
                    continue
                ...
                gLogger.progress(
                    "Loading TV show: %s, season %d..." % (title, seasonid)
                )

                if gConfig.QUERY_EPISODES:
                    (s3, t3, i3, data3) = jcomms.getData(
                        action,
                        "episodes",
                        filter,
                        extraFields,
        ...
        if mediatype == "tvshows":
            ...
            for tvshow in data:
                ...
                if newtvshow != {}:
                    ...
                    gLogger.out(
                        "Recently added TV show: %s (%d episode%s)"
                        % (tvshow.get("title"), epCount, "s"[epCount == 1 :]),
                        newLine=True,
                    )
            data = newData
        else:
            for item in data:
                gLogger.out(
                    "Recently added movie: %s"
                    % item.get("title", item.get("artist", item.get("name", None))),
                    newLine=True,
                )
    ...
    if data != []:
        if action == "cache":
            ...
            fileList = jcomms.getAllFilesForSource(
                mediatype, labels, gConfig.MISSING_IGNORE_PATTERNS, True
            )
...
# subsequently displayed to the user at the end.
#
def cacheImages(
    mediatype,
    jcomms,
    ...
    imagecache[
        ""
    ] = 0  # Ensure an empty image is already in the imagecache, with a zero reference count
    ...
    for item in mediaitems:
        if item.status in [MyMediaItem.STATUS_MISSING, MyMediaItem.STATUS_STALE]:
            ...
            if ITEMLIMIT == -1 or itemCount < ITEMLIMIT:
                ...
                MSG = "%s: [%-10s] for %s: %s\n" % (
                    reason,
                    item.itype.center(10),
                    re.sub("(.*)s$", "\\1", item.mtype),
                    item.getFullName(),
                )
                gLogger.out(MSG)
    ...
    unique_items = {}
    for item in mediaitems:
        if item.itype not in unique_items:
            ...
            for ui in sorted(unique_items):
                for item in mediaitems:
                    if item.dbid != 0 and item.itype == ui:
                        dbitem += 1
                        gLogger.progress(
                            "Pre-deleting cached items %d of %d... rowid %d, cachedurl %s"
                            % (dbitem, dbitems, item.dbid, item.cachedurl)
                        )
    ...
    for ui in sorted(unique_items):
        for item in mediaitems:
            if (
                item.status in [MyMediaItem.STATUS_MISSING, MyMediaItem.STATUS_STALE]
                and item.itype == ui
            ):
                c += 1
                    ...
                    for site in gConfig.SINGLETHREAD_URLS:
                        if site.search(item.decoded_filename):
                            ...
                            if gLogger.VERBOSE and gLogger.LOGGING:
                                gLogger.log(
                                    "QUEUE ITEM: single [%s], %s" % (site.pattern, item)
                                )
                ...
                gLogger.progress(
                    "Queueing work item: single thread %d, multi thread %d" % (sc, mc),
                    every=50,
                    finalItem=(c == itemCount),
                )
    ...
    if not multiple_work_queue.empty():
        ...
        gLogger.log(
            "Creating %d download thread(s) for multi-access sites" % THREADCOUNT
        )
    ...
    showProgress(
        threadcount,
        itemCount,
        single_work_queue.qsize(),
        multiple_work_queue.qsize(),
        error_queue.qsize(),
        itemsRemaining,
    )
    while threadcount > 0:
    ...
    if not error_queue.empty():
        ...
        while not error_queue.empty():
            ...
            if (
                item.mtype in ["sets", "movies", "tvshows", "seasons", "episodes"]
                and item.itype.find(".") == -1
            ):
...
def dump_drop_items(drop_items):
    if gConfig.CACHE_DROP_INVALID_FILE:
        ...
        outfile.write(
            json.dumps(drop_items.values(), indent=2, ensure_ascii=True, sort_keys=True)
        )
        outfile.close()
...
def showProgress(
    ...
    eta = (
        TOTALS.secondsToTime(remaining / tpersec, withMillis=False)
        if tpersec != 0.0
        else "**:**:**"
    )
    ...
    gLogger.progress(
        "Caching artwork: %d item%s remaining of %d (qs: %d, qm: %d), %d error%s, %d thread%s active%s"
        % (
            remaining,
            "s"[remaining == 1 :],
            maxItems,
...
def matchTextures(mediatype, mediaitems, jcomms, database, force, nodownload):
    ...
    if gConfig.CHUNKED:
        matchTextures_chunked(
            mediatype, mediaitems, jcomms, database, force, nodownload
        )
    else:
...
def matchTextures_chunked(mediatype, mediaitems, jcomms, database, force, nodownload):
    ...
    with database:
        ...
        for fnum, folder in enumerate(folders):
            ...
            gLogger.progress(
                "Loading Textures DB: chunk %2d of %d [unmatched %d: matched %d, skipped %d] (%d of %d)"
                % (fnum + 1, len(folders), unmatched, matched, skipped, dbindex, dbmax)
            )

            dbfiles = []
            for r in database.getRows(
                database.getTextureFolderFilter(folder), allfields=False
            ):
                dbfiles.append(r)
            ...
            for dbrow in dbfiles:
                ...
                if inum is not None:
                    ...
                    if item.status == MyMediaItem.STATUS_UNKNOWN:
                        unmatched -= 1
                        matchTextures_item_row(
                            mediatype, jcomms, item, dbrow, force, nodownload
                        )
...
def matchTextures_item_row(mediatype, jcomms, item, dbrow, force, nodownload):
    if item.mtype == "tvshows" and item.season == "Season All":
        TOTALS.bump("Season-all", item.itype)
...
def parseURLData(
    ...
    for item in data:
        ...
        gLogger.progress(
            "Parsing %s: %s..." % (mediatype.capitalize(), longName), every=25
        )
        ...
        if "art" in item:
            if SEASON_ALL and "poster" in item["art"]:
                SEASON_ALL = False
                (poster_url, fanart_url, banner_url) = jcomms.getSeasonAll(
                    item["art"]["poster"]
                )
        ...
        if "cast" in item:
            for a in item["cast"]:
                if "thumbnail" in a and evaluateURL(
                    "cast.thumb", a["thumbnail"], imagecache
                ):
                    mediaitems.append(
                        MyMediaItem(
                            mediatype,
...
def evaluateURL(imgtype, url, imagecache):
    ...
    if gConfig.CACHE_ARTWORK and imgtype not in gConfig.CACHE_ARTWORK:
        if gLogger.LOGGING:
            ...
            gLogger.log(
                "Ignored [%-12s] for [%s] as image type not in cache.artwork list"
                % (imgtype, decoded_url)
            )
    ...
    if gConfig.CACHE_IGNORE_TYPES:
        ...
        for ignore in gConfig.CACHE_IGNORE_TYPES:
            if ignore.search(decoded_url):
                gLogger.log(
                    "Ignored [%-12s] image due to [%s] rule: %s"
                    % (imgtype, ignore.pattern, decoded_url)
                )
                TOTALS.bump("Ignored", imgtype)
                imagecache[url] = 1
                return False
    ...
    for item in data:
                    ...
                    if MOD_MISSING_WARN_FAIL:
                        if gConfig.QA_FAIL_CHECKEXISTS and "file" in item:
                            if qa_check_artfile_exists(jcomms, mediatype, item, i):
                                ...
                                missing[
                                    "missing %s, local not found" % j
                                ] = gConfig.QA_FAIL_MISSING_LOCAL_ART
                        else:
                    ...
                    for qafailtype in gConfig.QA_FAIL_TYPES:
                        if qafailtype.search(decoded_url):
                            if gConfig.QA_FAIL_CHECKEXISTS and "file" in item:
                                if qa_check_artfile_exists(jcomms, mediatype, item, i):
                                    missing[
                                        "URL %s %s, local is available"
                                        % (j, qafailtype.pattern)
                                    ] = True
                                else:
                                    missing[
                                        "URL %s %s, local not found"
                                        % (j, qafailtype.pattern)
                                    ] = gConfig.QA_FAIL_MISSING_LOCAL_ART
                            else:
        ...
        if (check_file or nfo_file) and "file" in item:
            ...
            for file in MyUtility.unstackFiles(item["file"], addcombinedfile=True):
                ...
                data = jcomms.getDirectoryList(
                    dir, mediatype="files", properties=["file", "lastmodified"]
                )
                if files is None:
                ...
                if (
                    ...
                    and [
                        f
                        for f in files
                        if f["filetype"] == "file" and f.get("file", None) == file
                    ]
                ):
                ...
                if nfo_file and nfo_not_found:
                    ...
                    for f in [
                        x
                        for x in files
                        if x["filetype"] == "file" and x.get("file", None) == nfofile
                    ]:
                        nfo_not_found = False
                        jcomms.setTimeStamp(f)
                        if (
                            "lastmodified_timestamp" in f
                            and f["lastmodified_timestamp"]
                            >= gConfig.qa_nfo_refresh_date
                        ):
        ...
        if missing != {}:
            ...
            if "file" in item:
                mFAIL = "; ".join([x for x in missing if missing[x]])
                mWARN = "; ".join([x for x in missing if not missing[x]])
            else:
                mFAIL = ""
                mWARN = "; ".join(missing)
            ...
            msg = ""
            if mFAIL:
                msg = "%sFAIL (%s), " % (msg, mFAIL)
            if mWARN:
                msg = "%sWARN (%s), " % (msg, mWARN)
            msg = msg[:-2]
            ...
            if "file" in item and mFAIL:
                dir = "%s;%s" % (
                    mediatype,
                    os.path.dirname(MyUtility.unstackFiles(item["file"])[0]),
                )
...
def missingFiles(
    mediatype, data, fileList, title_name, id_name, showName=None, season=None
):
    gLogger.reset()
    ...
    for item in data:
        ...
        if "seasons" in item:
            missingFiles(
                "seasons", item["seasons"], fileList, "label", "season", showName=title
            )
        if "episodes" in item:
            missingFiles(
                "episodes",
    ...
    if showName is None:
        ...
        if fileList != []:
            gLogger.out(
                'The following media files are not present in the "%s" media library:\n\n'
                % mediatype
            )
...
def queryLibrary(
    ...
    for item in data:
                ...
                if temp is not None:
                    if type(temp) is list:
                        ...
                        if isinstance(temp, basestring):
                            temp = MyUtility.normalise(temp, strip=True)
        ...
        if MATCHED:
            mediaitems.append([name, DISPLAY[2:]])
    ...
    if mitems is None:
        ...
        for m in mediaitems:
            gLogger.out(
                "Matched: [%-50s] %s" % (addEllipsis(50, m[0]), m[1]), newLine=True
            )
...
def parseQuery(query):
    ...
    for value in newValue.split(" "):
        ...
        # Value
            ...
            tuples.append(
                [tField, tField.split("."), tCondition, INVERT, tValue, tLogic]
            )
            INVERT = False
        # Field name
        else:
...
def watchedWrite(filename, mediaitems):
    ...
    try:
        ...
        gLogger.out(
            "ERROR: failed to write the watched list to file [%s]" % filename,
            newLine=True,
        )
...
def watchedRead(filename, mediaitems):
    ...
    try:
        ...
        for m in MYLIST:
            ...
            mediaitems[mediakey] = MyWatchedItem(
                m["type"],
                m["name"],
                m["episode_year"],
                m["playcount"],
        ...
        gLogger.out(
            "ERROR: failed to read the watched list from file [%s]" % filename,
            newLine=True,
        )
        return False
...
def watchedBackup(
    ...
    for item in data:
        ...
        if playcount != 0 or lastplayed != "" or resume["position"] != 0.0:
            mediaitems.append(
                MyWatchedItem(
                    mediatype, shortName, episode_year, playcount, lastplayed, resume
                )
            )
...
def watchedRestore(
    ...
    if mitems is None:
        ...
        workItems = {}
        mediaitems = {}
        if not watchedRead(filename, mediaitems):
            return
    else:
        workItems = work
        mediaitems = mitems
            ...
            if mediakey in mediaitems:
                ...
                if m.libraryid == 0:
                    ...
                    if not gConfig.WATCHEDOVERWRITE:
                        m.refresh(
                            gConfig.JSON_HAS_SETRESUME, playcount, lastplayed, resume
                        )
    ...
    if mitems is None:
        ...
        for mediakey in sorted(mediaitems):
            m = mediaitems[mediakey]
            shortName = (
                "%s, Episode %s" % (m.name, m.episode_year)
                if m.mtype == "episodes"
                else "%s (%s)" % (m.name, m.episode_year)
            )
            if m.libraryid == 0:
                gLogger.out(
                    "NO MATCH %s: %s" % (m.mtype[:-1], shortName),
                    newLine=True,
                    log=True,
                )
                UNMATCHED += 1
                ...
                if m.state != 0:
                    ...
                    gLogger.out(
                        "Restored %s: %s" % (m.mtype[:-1], shortName),
                        newLine=True,
                        log=True,
                    )
                    RESTORED += 1
                    ...
                    gLogger.out(
                        "FAILED   %s: %s" % (m.mtype[:-1], shortName),
                        newLine=True,
                        log=True,
                    )
                    ERROR += 1
        gLogger.out("", newLine=True)
        gLogger.out(
            "Watched List item summary: restored %d, unchanged %d, unmatched %d, failed %d\n"
            % (RESTORED, UNCHANGED, UNMATCHED, ERROR),
            newLine=True,
        )
...
def updateIMDb(mediatype, jcomms, data):
    ...
    if "top250" in imdbfields:
        ...
        if movies250 is None:
            gLogger.err(
                "WARNING: failed to obtain Top250 movies, check log for details",
                newLine=True,
            )

    # Movies and TV shows
    worklist = _ProcessIMDB(
        mediatype, jcomms, data, plotFull, plotOutline, movies250, imdbfields
    )
    ...
    if mediatype == "tvshows":
        ...
        for tvshow in data:
            if tvshow["tvshowid"] not in tvhash:
                continue
            if "seasons" not in tvshow:
                continue
            ...
            for season in tvshow["seasons"]:
                ...
                for episode in season["episodes"]:
                    ...
                    if SxE == episode["label"] or int(SxE) < 1:
                        continue
...
def _ProcessIMDB(mediatype, jcomms, data, plotFull, plotOutline, movies250, imdbfields):
    ...
    if mediatype == "movies":
        ...
        for ft in gConfig.IMDB_MAP_TVTITLES:
            ftsplit = ft.split("=")
            if len(ftsplit) == 2:
                re_map_titles.append(
                    (re.compile(ftsplit[0], re.IGNORECASE), ftsplit[1])
                )

        for ft in gConfig.IMDB_TRANSLATE_TVTITLES:
            ftsplit = ft.split("=")
            if len(ftsplit) == 2:
                re_trans_titles.append(
                    (re.compile(ftsplit[0], re.IGNORECASE), ftsplit[1])
                )
        ...
        for ft in gConfig.IMDB_TRANSLATE_TVYEARS:
            ...
            if len(ftsplit) == 2 and ftsplit[1]:
                re_trans_years.append(
                    (re.compile(ftsplit[0], re.IGNORECASE), int(ftsplit[1]))
                )
        ...
        for tvshow in data:
            ...
            for ignore in re_ignore_titles:
                if ignore.search(tvshow["title"]):
                    gLogger.log(
                        "Pre-processing #1 OMDb: Title: [%s] ignoring TV show - matched [%s] in imdb.ignore.tvtitles"
                        % (tvshow["title"], ignore.pattern)
                    )
                    ignoreShow = True
                    break
            ...
            if not ignoreShow:
                ...
                for trans in re_map_titles:
                    if trans[0].search(tvshow["title"]):
                        gLogger.log(
                            "Pre-processing #2 OMDb: Title: [%s] mapped to imdbnumber for pattern [%s], assigning [%s]"
                            % (tvshow["title"], trans[0].pattern, trans[1])
                        )
                ...
                if "imdbnumber" in tvshow:
                    input_queue.put(
                        {"OriginalShowTitle": tvshow["tc.title"], "item": tvshow}
                    )
                    continue
                ...
                for trans in re_trans_years:
                    if trans[0].search(tvshow["title"]):
                        gLogger.log(
                            "Pre-processing #2 OMDb: Title: [%s] translating TV show year for pattern [%s], replacing [%d] with [%d]"
                            % (
                                tvshow["title"],
                                trans[0].pattern,
                                tvshow["year"],
                ...
                for trans in re_trans_titles:
                    if trans[0].search(tvshow["title"]):
                        ...
                        tvshow["title"] = (
                            trans[0].sub(trans[1], tvshow["title"]).strip()
                        )
                        gLogger.log(
                            "Pre-processing #3 OMDb: Title: [%s] translating TV show title for pattern [%s], replacing with [%s], giving [%s]"
                            % (
                                before_title,
                                trans[0].pattern,
                                trans[1],
                ...
                if tvshow["tc.year"] is not None and tvshow["title"].endswith(
                    "(%d)" % tvshow["tc.year"]
                ):
                    before_title = tvshow["title"]
                    tvshow["title"] = re.sub(
                        "\\(%d\\)$" % tvshow["tc.year"], "", tvshow["title"]
                    ).strip()
                    gLogger.log(
                        "Pre-processing #4 OMDb: Title: [%s] removing year from title, giving [%s]"
                        % (before_title, tvshow["title"])
                    )
                ...
                if gConfig.IMDB_DEL_PARENTHESIS:
                    ...
                    if re_find:
                        ...
                        gLogger.log(
                            "Pre-processing #5 OMDb: Title: [%s] removing trailing parenthesis from title, giving [%s]"
                            % (before_title, tvshow["title"])
                        )
    ...
    threadcount = (
        input_queue.qsize()
        if input_queue.qsize() <= gConfig.IMDB_THREADS
        else gConfig.IMDB_THREADS
    )
    threads = []
    for i in range(threadcount):
        t = MyIMDBLoader(
    ...
    while threadcount > 0:
        ...
        for field in imdbfields:
            ...
            if mediatype == "tvshows" and field in ["title"]:
                continue

            # Don't attempt to set genre for episodes - only works at the tvshow level
            if mediatype == "episodes" and field in ["genre"]:
                continue
            ...
            if field in newimdb:
                if (
                    field not in item
                    or item[field] != newimdb[field]
                    and newimdb[field] is not None
                ):
        ...
        if workitem["items"] or mediatype == "tvshows":
            ...
            if gLogger.LOGGING:
                with lock:  # Grab threading lock to prevent threads intermingling their output with our main thread
                    gLogger.log(
                        "Workitem for id: %d, type: %s, title: %s"
...
def getIntFloatStr(aField, aValue):
    ...
    isString = (
        type(aValue) is str if MyUtility.isPython3 else type(aValue) in [str, unicode]  # noqa: F821
    )
    if isString:
        if (aValue.startswith('"') and aValue.endswith('"')) or (
            aValue.startswith("'") and aValue.endswith("'")
...
def setDetails_worker(
    jcomms, mtype, libraryid, kvpairs, title, dryRun, itemnum, maxitems, typeconversion
):
    if mtype == "movie":
        method = "VideoLibrary.SetMovieDetails"
        idname = "movieid"
        ...
        if gConfig.JSON_HAS_SETMOVIESET:
            ...
            gLogger.out(
                "WARNING: %s is not supported by this version of JSON API (%s) - ignored"
                % (mtype, gConfig.JSON_VER_STR),
                newLine=True,
            )
            return
    elif mtype == "tvshow":
        ...
        gLogger.out(
            "ERROR: %s is not a valid media type for this operation (id: %d)"
            % (mtype, libraryid),
            newLine=True,
        )
        return
        ...
        if bKEY:
            ...
            if pair is None:
                ...
                for item in pair:
                    if item:
                        pairs[KEY].append(
                            getIntFloatStr(KEY, item) if typeconversion else item
                        )
            elif (
                (isinstance(pair, basestring))
                and pair.startswith("[")
                and pair.endswith("]")
            ):
                pairs[KEY] = []
            ...
            if (
                (pairs[KEY] is None or pairs[KEY] == "")
                and (KEY.startswith("art.") or KEY in ["fanart", "thumbnail", "thumb"])
                and not gConfig.JSON_HAS_SETNULL
            ):
                value = "null" if pairs[KEY] is None else '"%s"' % pairs[KEY]
                gLogger.out(
                    "WARNING: cannot set null/empty string value on field with "
                    "JSON API %s - ignoring %s %6d (%s = %s)"
                    % (gConfig.JSON_VER_STR, idname, libraryid, KEY, value),
                    newLine=True,
                    log=True,
                )
                return
    ...
    if dryRun:
        ...
        gLogger.out(
            json.dumps(REQUEST, indent=2, ensure_ascii=True, sort_keys=False),
            newLine=True,
        )
        gLogger.out("### DRY RUN ###", newLine=True)
    else:
...
def sqlExtract(ACTION="NONE", search="", filter="", delete=False, silent=False):
    ...
    with database:
        ...
        if search != "":
            SQL.append(
                "WHERE t.url LIKE '%%%s%%' ORDER BY t.cachedurl ASC"
                % search.replace("'", "''")
            )
            ...
            if gConfig.SEARCH_ENCODE:
                ...
                if search2 != search:
                    ...
                    SQL.append(
                        "WHERE t.url LIKE '%%%s%%'%s ORDER BY t.cachedurl ASC"
                        % (search2, ESCAPE)
                    )
        elif filter != "":
            SQL.append("%s " % filter)
        ...
        for row in dbrows:
            if ACTION == "NONE":
                ...
                i += 1
                gLogger.progress(
                    "Parsing [%s] %2.0f%%..." % (row["cachedurl"], rpcnt * i), every=50
                )
                if ACTION == "EXISTS":
        ...
        if delete:
            ...
            for row in ROWS:
                i += 1
                gLogger.progress(
                    "Deleting row %d (%d of %d)..." % (row["textureid"], i, FCOUNT)
                )
                database.deleteItem(
                    row["textureid"], row["cachedurl"], warnmissing=False
                )
                gLogger.progress("")
        elif not silent:
            for row in ROWS:
...
def orphanCheck(removeOrphans=False):
    ...
    for (root, dirs, files) in os.walk(path):
        ...
        for file in files:
            ...
            if not row:
                ...
                gLogger.log(
                    "Orphan file detected: [%s] with likely hash [%s]"
                    % (filename, hash)
                )
    ...
    if (
        removeOrphans
        and gConfig.ORPHAN_LIMIT_CHECK
        and len(orphanedfiles) > (len(dbfiles) / 20)
    ):
        gLogger.log(
            "Something is wrong here, that's far too many orphaned files - 5% limit exceeded!"
        )
        gLogger.out(
            "Found %s orphaned files for %s database files.\n\n"
    ...
    for ofile in orphanedfiles:
        ...
        FSIZE += fsize
        gLogger.out(
            "Orphaned file found: name [%s], created [%s], size [%s]%s\n"
            % (
                ofile,
...
def pruneCache(remove_nonlibrary_artwork=False):
    ...
    if localfiles != []:
        if remove_nonlibrary_artwork:
            ...
            gLogger.out(
                "The following items are present in the texture cache but not the media library:",
                newLine=True,
            )
        gLogger.out("", newLine=True)
...
def pruneCache_fast(database, libraryFiles, mediaFiles, localfiles, re_search):
    ...
    for rownum, hash in enumerate(dbfiles):
        gLogger.progress(
            "Processing texture cache... %d%%" % (100 * rownum / totalrows), every=25
        )
        pruneCache_processrow(
            dbfiles[hash], libraryFiles, mediaFiles, localfiles, re_search
        )
...
def pruneCache_chunked(database, libraryFiles, mediaFiles, localfiles, re_search):
    ...
    with database:
        ...
        for fnum, folder in enumerate(folders):
            gLogger.progress(
                "Loading Textures DB: chunk %2d of %d..." % (fnum + 1, len(folders))
            )

            # dbfiles = []
            dbrows = database.getRows(
                database.getTextureFolderFilter(folder), allfields=True
            )
            ...
            for dbrow in dbrows:
                ...
                gLogger.progress(
                    "Processing artwork: chunk %2d of %d (%d%%)"
                    % (fnum + 1, len(folders), (100 * i / j)),
                    every=25,
                    finalItem=(i == j),
                )

                pruneCache_processrow(
                    dbrow, libraryFiles, mediaFiles, localfiles, re_search
                )
...
def getAllFiles(keyFunction):
        ...
        for items in data.get("result", {}):
            if items != "limits":
                ...
                for i in data["result"][items]:
                    ...
                    gLogger.progress(
                        "Loading %s: %s..." % (mediatype, title), every=interval
                    )
                    if "fanart" in i:
                        afiles[keyFunction(i["fanart"])] = "fanart"
                    if "thumbnail" in i:
                        afiles[keyFunction(i["thumbnail"])] = "thumbnail"
                ...
                if title != "":
                    gLogger.progress("Parsing %s: %s..." % (mediatype, title))
            ...
            if "seasons" in seasondata["result"]:
                ...
                for season in seasondata["result"]["seasons"]:
                    ...
                    if seasonid < 0:
                        gLogger.err(
                            "WARNING: TV show [%s] has invalid season (%d) - ignored"
                            % (tvshow["title"], seasonid),
                            newLine=True,
                        )
                        continue

                    gLogger.progress(
                        "Loading TV show: %s, season %d..."
                        % (tvshow["title"], seasonid)
                    )

                    for a in season.get("art", {}):
                        if SEASON_ALL and a in [
                            "poster",
                            "tvshow.poster",
                            "tvshow.fanart",
                            "tvshow.banner",
                            ...
                            if poster_url:
                                afiles[keyFunction(poster_url)] = "poster"
                            if fanart_url:
                                afiles[keyFunction(fanart_url)] = "fanart"
                            if banner_url:
                                afiles[keyFunction(banner_url)] = "banner"
                    ...
                    episodedata = jcomms.getDataProxy(
                        "episodes", REQUEST, uniquecast=UCAST
                    )
    ...
    pictures = jcomms.getPictures(
        addPreviews=gConfig.PRUNE_RETAIN_PREVIEWS,
        addPictures=gConfig.PRUNE_RETAIN_PICTURES,
    )
...
def removeMedia(mtype, libraryid):
    ...
    if mtype not in MTYPE:
        gLogger.out(
            "ERROR: %s is not a valid media type for removal - valid types: %s"
            % (mtype, ", ".join(MTYPE)),
            newLine=True,
        )
        return
    ...
    if libraryid < 1:
        gLogger.out(
            "ERROR: %d is not a valid libraryid for removal" % libraryid, newLine=True
        )
        return
    ...
    if title:
        ...
        gLogger.out(
            "ERROR: does not exist - media type [%s] libraryid [%d]"
            % (mtype, libraryid),
            newLine=True,
        )
    ...
    with database:
        for pattern in [x for x in patterns if x != ""]:
            if len(pattern.replace("%", "")) < gConfig.PURGE_MIN_LEN:
                gLogger.err(
                    "Ignoring [%s] as pattern length (excluding wildcards) is less than "
                    "%d characters configured by purge.minlen property"
                    % (pattern, gConfig.PURGE_MIN_LEN),
                    newLine=True,
                )
                continue
            ...
            rows = database.getRows(
                filter=(SQL % sqlpattern), order="ORDER BY t.id ASC", allfields=True
            )
            ...
            if gConfig.USEJSONDB and not gConfig.JSON_HAS_FILTERNULLVALUE:
                ...
                for r in rows:
                    if (
                        (hashType == "all")
                        or (hashType == "hashed" and r["lasthashcheck"])
                        or (hashType == "unhashed" and not r["lasthashcheck"])
                    ):
                        newrows.append(r)
                rows = newrows
                newrows = None
            ...
            gLogger.out(
                "Purging %d (%s) items for pattern: %s"
                % (len(rows), hashType, pattern),
                newLine=True,
            )
            ...
            for r in rows:
                if dryRun:
                    ...
                    database.deleteItem(
                        r["textureid"], r["cachedurl"], warnmissing=False
                    )
        ...
        for items in data.get("result", {}):
            if items != "limits":
                ...
                for i in data["result"][items]:
                    title = i.get("title", i.get("artist", i.get("name", None)))
                    gLogger.progress(
                        "Parsing %s: %s..." % (mediatype, title), every=interval
                    )
...
def getDirectory(path, recurse=False):
    getDirectoryFiles(
        MyJSONComms(gConfig, gLogger), path, nodirmsg=True, recurse=recurse
    )
...
def getDirectoryFiles(jcomms, path, nodirmsg=True, recurse=False):
    ...
    for file in sorted(
        data["result"]["files"], key=(lambda x: x["file"]), reverse=False
    ):
        if "file" in file:
            ftype = file["filetype"]
            fname = file["file"]
...
def showStatus(idleTime=600):
    ...
    if "result" in data:
        STATUS.append(
            "Scanning Music: %s"
            % ("Yes" if data["result"].get("Library.IsScanningMusic", False) else "No")
        )
        STATUS.append(
            "Scanning Video: %s"
            % ("Yes" if data["result"].get("Library.IsScanningVideo", False) else "No")
        )
        STATUS.append(
            "ScreenSaver Active: %s"
            % ("Yes" if data["result"].get("System.ScreenSaverActive", False) else "No")
        )
        STATUS.append(
            "Suspend Supported: %s"
            % ("Yes" if data["result"].get("System.CanSuspend", False) else "No")
        )
        STATUS.append(
            "Idle Timer Enabled: %s"
            % ("Yes" if data["result"].get("System.HasShutdown", False) else "No")
        )
    ...
    if "result" in data:
        STATUS.append(
            "System Idle > %ss: %s"
            % (idleTime, ("Yes" if data["result"].get(property, False) else "No"))
        )
            ...
            for player in data["result"]:
                if "playerid" in player:
                    ...
                    if "result" in data and "item" in data["result"]:
                        ...
                        data = jcomms.sendJSON(
                            REQUEST, "libGetProps", checkResult=False
                        )
                        if "result" in data:
                            ...
                            if data["result"].get("speed", 0) == 0:
                                STATUS.append(
                                    "Progress: %s (%4.2f%%, %s remaining, paused)"
                                    % (elapsed, pcnt, remaining)
                                )
                            elif data["result"].get("speed", 0) != 1:
                                STATUS.append(
                                    "Progress: %s (%4.2f%%, %s remaining, seeking)"
                                    % (elapsed, pcnt, remaining)
                                )
                            else:
                                STATUS.append(
                                    "Progress: %s (%4.2f%%, %s remaining)"
                                    % (elapsed, pcnt, remaining)
                                )
...
def rbphdmi(delay):
    ...
    while True:
        try:
            MyJSONComms(gConfig, gLogger).sendJSON(
                {"method": "JSONRPC.Ping"},
                "libListen",
                callback=rbphdmi_listen,
                checkResult=False,
            )
            if RETRIES != 0:
            ...
            gLogger.debug(
                "Kodi not responding, retries remaining %d" % (RETRIES - ATTEMPTS)
            )
            if ATTEMPTS >= RETRIES:
                raise
            time.sleep(5.0)
...
def MediaLibraryStats(media_list):
    ...
    if "video" in lmedia_list:
        lmedia_list.extend(
            ["movies", "moviesets", "tvshows", "episodes", "musicvideos"]
        )
    ...
    if gConfig.JSON_HAS_PROFILE_SUPPORT:
        gLogger.out(
            "%-11s: %s" % ("Profile", gConfig.CURRENT_PROFILE["label"]), newLine=True
        )
...
def ProcessInput(args):
    ...
    if len(actionparam) > 0:
        if not (
            actionparam[0]["action"] == "Pause"
            or (
                actionparam[0]["action"] == "ExecuteAction"
                and actionparam[0]["param"] == "noop"
            )
        ):
    ...
    for ap in actionparam:
        ...
        if "result" not in data or data["result"] != "OK":
            gLogger.err(
                "Unexpected error during [%s] with params [%s]"
...
def StressTest(viewtype, numitems, pause, repeat, cooldown):
    ...
    if viewtype == "thumbnail":
        ...
        if TOTAL_ROWS > 0:
            for i in range(0, TOTAL_ROWS):
                ...
                COMMANDS = "%s%s" % (
                    COMMANDS,
                    st_move_horizontal(d, (gConfig.POSTER_WIDTH - 1), pause),
                )
        ...
        gLogger.err(
            "%s is not a valid viewtype for stress-test" % viewtype, newLine=True
        )
        sys.exit(2)
    ...
    for i in range(0, repeat):
        ...
        gLogger.out(
            "Loop %4d of %d, %s over %d GUI items with %s second pause..."
            % (i + 1, repeat, viewtype, numitems, pause),
            padspaces=False,
        )
...
def st_move_right(count, pause):
    return (
        "right pause %s " % pause
        if count == 1
        else st_move_horizontal("right", count, pause)
    )
...
def st_list_move(direction, count, pause):
    ...
    if count > 0:
        for i in range(0, count):
            cmd = "%s%s" % (
                cmd,
                st_move_down(pause) if direction == "down" else st_move_right(1, pause),
            )
    return cmd
...
def setVolume(volume):
    if volume == "mute":
        ...
        try:
            ...
            gLogger.err(
                "ERROR: volume level [%s] is not a valid integer" % volume, newLine=True
            )
            return

    data = MyJSONComms(gConfig, gLogger).sendJSON(
        REQUEST, "libVolume", checkResult=False
    )
    if "result" not in data:
        gLogger.err(
            "ERROR: volume change failed - valid values: 0-100, mute and unmute",
            newLine=True,
        )
...
def readFile(infile, outfile):
    ...
    if url:
        try:
            ...
            gLogger.err(
                "ERROR not authorised access to file: %s" % infile, newLine=True
            )
            sys.exit(2)
        except Exception as e:
...
def getSettingVariable(name):
    ...
    data = MyJSONComms(gConfig, gLogger).sendJSON(
        REQUEST, "libGetSetting", checkResult=True
    )
...
def ReadSettings(pattern=None):
    ...
    data = MyJSONComms(gConfig, gLogger).sendJSON(
        REQUEST, "libSettings", checkResult=True
    )
    if pattern:
        newdata = []
        for item in data["result"]["settings"]:
            if item["id"].find(pattern) != -1:
                newdata.append(item)
        gLogger.out(
            json.dumps(newdata, indent=2, ensure_ascii=True, sort_keys=True),
            newLine=True,
        )
    else:
        gLogger.out(
            json.dumps(
                data["result"]["settings"], indent=2, ensure_ascii=True, sort_keys=True
            ),
            newLine=True,
        )
...
def playerPlay(afile, playerid=None, withWait=False):
    ...
    if playerid is not None:
        ...
        if playercoreid == "null":
            ...
            if not gConfig.JSON_HAS_OPEN_PLAYERCORE_DEFAULT:
                gLogger.err(
                    "WARNING: 'default' is not supported by current JSON API",
                    newLine=True,
                )
                playercoreid = None
        else:
    ...
    MyJSONComms(gConfig, gLogger).sendJSON(
        ...
        "libPlayer",
        checkResult=True,
        callback=_playlisten if withWait else None,
    )
...
def usage(EXIT_CODE):
    ...
    print("")
    pprint(
        "[s, S] <string> | [x, X, f, F] [sql-filter] | Xd | d <id[id id]>] | \
...
def checkConfig(option):
    ...
    if needWeb:
        try:
            wcomms = MyJSONComms(
                gConfig, gLogger, connecttimeout=gConfig.WEB_CONNECTTIMEOUT
            )
            REQUEST = {"method": "JSONRPC.Ping"}
            data = wcomms.sendJSON(
                REQUEST, "libPing", checkResult=False, useWebServer=True
            )
    ...
    if needSocket or trySocket:
        try:
            jcomms = MyJSONComms(
                gConfig, gLogger, connecttimeout=gConfig.RPC_CONNECTTIMEOUT
            )
            ...
            if "result" in data and "version" in data["result"]:
                ...
                if type(jsonGotVersion) is dict and "major" in jsonGotVersion:
                    gConfig.SetJSONVersion(
                        jsonGotVersion.get("major", 0),
            ...
            if gConfig.JSON_HAS_PROFILE_SUPPORT:
                ...
                if (
                    gConfig.PROFILE_ENABLED
                    and gConfig.CURRENT_PROFILE["label"] != gConfig.PROFILE_NAME
                    and option != "profiles"
                ):
                    if not switchprofile(jcomms):
                        return False
                    jcomms = MyJSONComms(gConfig, gLogger)
            ...
            gConfig.HAS_PVR = "result" in data and data["result"].get(
                "available", False
            )
    ...
    if needDb and gConfig.DBJSON == "auto":
        ...
        if gConfig.JSON_HAS_TEXTUREDB:
            ...
            gLogger.log(
                "JSON Textures DB API available and will be used to access the Textures DB"
            )
        else:
            gConfig.USEJSONDB = False
            gLogger.log(
                "JSON Textures DB API not supported - will use SQLite to access the Textures DB"
            )
    ...
    if needMAC and not gotMAC:
        MSG = (
            "FATAL: The task you wish to perform requires a valid MAC address\n"
...
def loadprofile(jcomms):
    REQUEST = {
        ...
        "params": {"profile": gConfig.PROFILE_NAME, "prompt": False},
    }
    if gConfig.PROFILE_PASSWORD != "":
        REQUEST["params"]["password"] = {"value": gConfig.PROFILE_PASSWORD}
        REQUEST["params"]["password"]["encryption"] = (
            "md5" if gConfig.PROFILE_ENCRYPTED else "none"
        )
    ...
    data = jcomms.sendJSON(
        REQUEST, "libProfile", checkResult=False, ignoreSocketError=True
    )
    if "result" not in data:
        return False
    else:
...
def getallprofiles(jcomms):
    ...
    if "result" in data:
        master = [
            p
            for p in data["result"]["profiles"]
            if p["label"] == gConfig.PROFILE_MASTER
        ]
...
def getcurrentprofile(jcomms):
    ...
    if "result" in data:
        profile = data["result"]
        master = gConfig.ALL_PROFILES.get(
            gConfig.PROFILE_MASTER, gConfig.ALL_PROFILES.get("Master user", None)
        )
        setProfileDirectory(master, profile)
        return profile
    else:
...
def setProfileDirectory(master, profile):
    ...
    if not os.path.isabs(profile["tc.profilepath"]):
        profile["tc.profilepath"] = os.path.join(
            gConfig.KODI_BASE, profile["tc.profilepath"]
        )
...
def switchprofile(jcomms):
    if gConfig.CURRENT_PROFILE["label"] == gConfig.PROFILE_NAME:
        ...
        gLogger.log(
            'SWITCHING PROFILE FROM "%s" to "%s"'
            % (gConfig.CURRENT_PROFILE["label"], gConfig.PROFILE_NAME)
        )
        gLogger.progress(
            'Switching profile from "%s" to "%s"...'
            % (gConfig.CURRENT_PROFILE["label"], gConfig.PROFILE_NAME)
        ...
        if not loadprofile(jcomms):
            gLogger.err(
                'ERROR: Profile "%s" is not valid!' % gConfig.PROFILE_NAME, newLine=True
            )
            return False
        ...
        while i <= gConfig.PROFILE_RETRY:
            try:
                ...
                if gConfig.CURRENT_PROFILE["label"] == gConfig.PROFILE_NAME:
                    ...
                    if gConfig.PROFILE_WAIT != 0:
                        gLogger.log(
                            "Waiting %d seconds for server to stabilise after loading profile..."
                            % gConfig.PROFILE_WAIT
                        )
            ...
            gLogger.err(
                "ERROR: Failed to load profile %s" % gConfig.PROFILE_NAME, newLine=True
            )
            return False
        gLogger.progress("")
    else:
        gLogger.err(
            'ERROR: Need to switch profiles from "%s" to "%s", but profile.autoload is not enabled'
            % (gConfig.CURRENT_PROFILE["label"], gConfig.PROFILE_NAME),
            newLine=True,
        )
        return False
...
def listProfiles():
    print(
        "%s  %s  %-20s  %-50s  %s"
        % ("Active", "Lock", "Name", "Device Path", "Local Path")
    )
    for p in gConfig.ALL_PROFILES:
        profile = gConfig.ALL_PROFILES[p]
        active = (
            "Yes" if profile["label"] == gConfig.CURRENT_PROFILE["label"] else "No "
        )
...
def checkUpdate(argv, forcedCheck=False):
    ...
    if forcedCheck:
        ...
        gLogger.out(
            "Latest  Version: %s"
            % ("v" + remoteVersion if remoteVersion else "Unknown"),
            newLine=True,
        )
        gLogger.out("", newLine=True)

    if remoteVersion and MyUtility.getVersion(remoteVersion) > MyUtility.getVersion(
        gConfig.VERSION
    ):
...
def getLatestVersion(argv):
    ...
    if remoteVersion is None or remoteHash is None:
        (remoteVersion, remoteHash) = getLatestVersion_ex(
            "%s/%s" % (gConfig.GITHUB, "VERSION")
        )
...
def getLatestVersion_ex(url, headers=None):
    ...
    try:
        ...
        if len(items) == 2:
            ...
            gLogger.log(
                "Bogus data in getLatestVersion_ex(): URL [%s], data [%s]"
                % (url, data),
                maxLen=512,
            )
    except Exception as e:
        gLogger.log(
            "Exception in getLatestVersion_ex(): URL [%s], text [%s]" % (url, e)
        )
...
def downloadLatestVersion(argv, force=False, autoupdate=False):
    ...
    if autoupdate and (
        not remoteVersion
        or MyUtility.getVersion(remoteVersion) <= MyUtility.getVersion(gConfig.VERSION)
    ):
        return False

    if not remoteVersion:
        gLogger.err(
            "FATAL: Unable to determine version of the latest file, check Internet access and github.com are available.",
            newLine=True,
        )
        sys.exit(2)

    if not force and MyUtility.getVersion(remoteVersion) <= MyUtility.getVersion(
        gConfig.VERSION
    ):
        gLogger.err(
            "Current version is already up to date - no update required.", newLine=True
        )
        sys.exit(2)
    ...
    try:
        ...
        if autoupdate:
            return False
        gLogger.err(
            "FATAL: Unable to download latest file, check Internet access and github.com are available.",
            newLine=True,
        )
        sys.exit(2)
    ...
    if digest.hexdigest() != remoteHash:
        if autoupdate:
            return False
        gLogger.err(
            "FATAL: Checksum of new version is incorrect, possibly corrupt download - abandoning update.",
            newLine=True,
        )
        sys.exit(2)
    ...
    if os.path.exists("%s%s.git" % (dir, os.sep)):
        gLogger.err(
            "FATAL: Might be updating version in Git repository... Abandoning update!",
            newLine=True,
        )
        sys.exit(2)
    ...
    try:
        ...
        if autoupdate:
            gLogger.err(
                "NOTICE: A new version (v%s) of this script is available."
                % remoteVersion,
                newLine=True,
            )
            gLogger.err(
                'NOTICE: Use the "--update" option to apply update.', newLine=True
            )
            return False
        else:
            gLogger.err(
                "FATAL: Unable to update current file, check you have write access",
                newLine=True,
            )
            sys.exit(2)

    gLogger.err(
        "Successfully updated from v%s to v%s" % (gConfig.VERSION, remoteVersion),
        newLine=True,
    )
...
def main(argv):
    ...
    multi_call = (
        ["addons", "agenres", "vgenres", "pvr.tv", "pvr.radio"]
        + multi_call_a
        + multi_call_v
    )
    ...
    if argv[0] == "s" and len(argv) == 2:
        ...
        if _force and not gConfig.RECACHEALL and _filter == "":
            print(
                'Forcing re-cache of all items is disabled. Enable by setting "allow.recacheall=yes" in property file.'
            )
            sys.exit(2)
        ...
        if len(argv) == 1:
            ...
            if "songs" in _multi_call:
                _multi_call.remove("songs")
        else:
            if argv[1] == "audio":
                _multi_call = multi_call_a
            if argv[1] == "video":
                _multi_call = multi_call_v
            if argv[1] == "all":
                _multi_call = multi_call
        ...
        if _multi_call != [] and not gConfig.HAS_PVR:
            for item in ["pvr.tv", "pvr.radio"]:
                if item in _multi_call:
                    _multi_call.remove(item)
        ...
        if _multi_call != []:
            ...
            if _action == "cache":
                dump_drop_items(_drop_items)
            if _stats:
                TOTALS.libraryStats(
                    multi=_multi_call, filter=_filter, lastRun=_lastRun, query=_query
                )
        else:
            usage(1)
        ...
        if argv[1] not in ["hashed", "unhashed", "all"]:
            usage(1)
