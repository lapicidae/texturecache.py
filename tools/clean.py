...
def processitems(data, keepart, patterns):
    ...
    for item in data:
        ...
        for a in item.get("art", []):
            if (
                not (a.startswith("tvshow.") or a.startswith("season."))
                and a not in keepart
            ):
                if not patterns:
                    debug(
                        "Not Keep: [%-12s] %-50s [%s]"
                        % (
                            a,
                            addEllipsis(50, item.get("title", item.get("label", None))),
                            item["art"][a],
                        )
                    )
                    items["art.%s" % a] = None
                        ...
                        if pattern.search(url):
                            debug(
                                ...
                                % (
                                    a,
                                    addEllipsis(
                                        50, item.get("title", item.get("label", None))
                                    ),
                                    item["art"][a],
                                )
                            )
        ...
        if "seasons" in item:
            workitems.extend(processitems(item["seasons"], keepart, patterns))
        if "episodes" in item:
            workitems.extend(processitems(item["episodes"], keepart, patterns))
...
def init():
    ...
    parser = argparse.ArgumentParser(
        description="Generate instructions to remove artwork URLs matching specific patterns \
    ...
    if args.keepart is None and args.patterns is None:
