...
class MyUtility(object):
    ...
    def toUnicode(value):
        ...
        if isinstance(value, basestring):  # noqa: F821
            if not isinstance(value, unicode):  # noqa: F821
                try:
                    value = unicode(  # noqa: F821
                        value, encoding="utf-8", errors="ignore"
                    )
                except UnicodeDecodeError:
                    pass
...
def info(args, msg, atype, title, reason=None, url=None, target=None):
    ...
    if reason or url:
        line = "%s%-15s - %-10s - %-45s" % (
            ...
            msg,
            atype.center(10),
            addEllipsis(45, MyUtility.toUnicode(title)),
        )
        if reason:
            line = "%s [%s]" % (line, reason)
        if url:
            line = "%s %s" % (line, MyUtility.toUnicode(url))
    elif target:
        line = "%s%-15s - %-10s - %-45s -> %s" % (
            line,
            msg,
            atype.center(10),
            addEllipsis(45, MyUtility.toUnicode(title)),
            MyUtility.toUnicode(target),
        )
    else:
        line = "%s%-15s - %-10s - %s" % (
            line,
...
def debug(indent, msg):
    global VERBOSE
    if VERBOSE:
        printerr("##DEBUG## %s%s" % (" " * (indent * 2), msg))
        ...
        if value1:
            debug(
                ...
                % (
                    atype,
                    msg,
                    MyUtility.toUnicode(value1),
                    MyUtility.toUnicode(value2),
                ),
            )
        else:
...
def itemList(aList):
    ...
    if aList:
        for aname in aList:
            (aname, value) = (
                (aname, aname) if aname.find(":") == -1 else aname.split(":")
            )
...
def findMostFrequentSetParent(setname, members, level=0, counts=None):
    ...
    if level == 0:
        ...
        sorted_counts = [
            x for x in sorted(counts, key=len, reverse=True) if counts[x] == maxfreq
        ]

        return "%s%s" % (sorted_counts[0], getSlash(sorted_counts[0]))


def formatArtworkFilename(
    args, mediatype, filename, suffix, season, singleFolder=False
):
    ...
    if mediatype == "movie":
        if suffix == "fanart#":
            parent = (
                args.extrafanart
                if args.extrafanart
                else "%s%s%s" % (os.path.dirname(filename), joinchar, "extrafanart")
            )
            ...
            parent = (
                args.extrathumbs
                if args.extrathumbs
                else "%s%s%s" % (os.path.dirname(filename), joinchar, "extrathumbs")
            )
...
def processItem(args, mediatype, media, download_items, showTitle=None, showPath=None):
    ...
    if XBMC_PATH and not mediafile.startswith(XBMC_PATH):
        if not args.ignorebadprefix:
            warning(
                ...
                "Bad prefix",
                mediatitle,
                "Kodi path does not match prefix",
                mediafile,
            )
        return workitem
    ...
    for artitem in download_items:
        if artitem["type"] in previtems:
            debug(
                1,
                "already found a value for artwork type [%s] - ignoring repeated item"
                % artitem["type"],
            )
            continue

        artpath_m = formatArtworkFilename(
            args,
            mediatype,
            filename,
            artitem["suffix"],
            media.get("season", None),
            singleFolder=False,
        )
        artpath_s = formatArtworkFilename(
            args,
            mediatype,
            filename,
            artitem["suffix"],
        ...
        if artitem["type"] in ["fanart#", "thumb#"]:
            if mediatype not in ["movie", "tvshow"]:
                debug(
                    1,
                    "invalid artwork type [%s] for media [%s] - ignoring"
                    % (artitem["type"], mediatype),
                )
                continue

            m_regex = re.compile("%s[0-9]+" % artitem["type"][:-1])
            m_max = (
                args.extrafanartmax + 1
                if artitem["type"] == "fanart#"
                else args.extrathumbsmax + 1
            )
            ...
            for index in range(1, m_max):
                m_items[index] = False
            for aitem in [
                MyUtility.extract_index(artitem["type"], x)
                for x in art
                if m_regex.match(x)
            ]:
            ...
            for m_index in sorted(m_items):
                ...
                if m_type in previtems:
                    debug(
                        1,
                        "already found a value for artwork type [%s] - ignoring repeated item (expanded from %s)"
                        % (m_type, artitem["type"]),
                    )
                    continue
                ...
                if oldname:
                    oldname = MyUtility.toutf8(oldname[8:-1])

                newname = processArtwork(
                    args,
                    mediatype,
                    media,
                    mediatitle,
            ...
            updateworkitem(
                args, artitem["type"], mediatitle, workitem, oldname, newname
            )
    ...
    if args.check:
        clist = (
            args.check
            if args.check != ["all"]
            else [x for x in art if not x.startswith("tvshow.")]
        )
...
def updateworkitem(args, art_type, mediatitle, workitem, oldname, newname):
    ...
    if not newname and oldname:
        ...
        if newname and newname != oldname:
            ...
            if label in workitem["items"]:
                del workitem["items"][label]
...
def processArtwork(
    ...
    if args.readonly:
        if args.nokeep:
            ...
            warning(
                args, "**  NEEDED  **", atype, title, "readonly enabled", currentname
            )
            return currentname
    ...
    fname = pathToXBMC(
        getImage(args, mediatype, media, title, atype, filename, currentname, target)
    )
...
def getImage(args, mediatype, media, title, atype, filename, source, target):
    ...
    if source in NOT_AVAILABLE_CACHE:
        ...
        warning(
            ...
            atype,
            title,
            "Prior download failed: %4d" % NOT_AVAILABLE_CACHE[source],
            source,
        )
        return None
    ...
    if not source.startswith("http://") and not source.startswith("https://"):
        ...
        debug2(
            atype,
            "Lookup non-HTTP file using current URL:",
            newsource,
            (" [%s]" % ("SUCCESS" if found_file else "FAIL")),
        )
        ...
        if LOCAL_ALT and not found_file:
            ...
            newsource = formatArtworkFilename(
                mediatype,
                pathToAltLocal(os.path.splitext(filename)[0]),
                atype,
                media.get("season", None),
                singleFolder=args.singlefolders,
            )
            ...
            if newsource != currentsource:
                ...
                debug2(
                    atype,
                    "Lookup non-HTTP file using altlocal path:",
                    newsource,
                    (
                        " [%s] (based on media filename)"
                        % ("SUCCESS" if found_file else "FAIL")
                    ),
                )
        ...
        if found_file:
            if not args.dryrun:
                if newsource != target:
                    ...
                    debug2(
                        atype,
                        ("Copied %d bytes of data:" % os.path.getsize(target)),
                        target,
                    )
                else:
                    debug2(
                        atype,
                        "WARNING: Target is same as source - skipping copy:",
                        target,
                    )
            else:
            ...
            info(
                args,
                "%9d bytes" % os.path.getsize(newsource),
                atype,
                title,
                None,
                None,
...
def showConfig(args, download_items, season_items, episode_items):
    ...
    printerr(
        "  Extra Fanart  : %s (max: %d)"
        % (_blank(args.extrafanart), args.extrafanartmax)
    )
    printerr(
        "  Extra Thumbs  : %s (max: %d)"
        % (_blank(args.extrathumbs), args.extrathumbsmax)
    )
...
def init():
    ...
    parser = argparse.ArgumentParser(
        description="Downloads specific artwork types (default: clearart, clearlogo) \
    ...
    parser.add_argument(
        "--info", action="store_true", help="Display informational output to stdout"
    )
    ...
    group.add_argument(
        "-v", "--verbose", action="store_true", help="Display diagnostic output"
    )
    ...
    if args.season == []:
        args.season = ["poster", "banner", "landscape"]
    if args.episode == []:
        args.episode = ["thumb"]

    LOCAL_DIR = args.local
    if LOCAL_DIR and LOCAL_DIR[-1] not in ["/", "\\"]:
        LOCAL_DIR += getSlash(LOCAL_DIR)

    LOCAL_ALT = args.altlocal
    if LOCAL_ALT and LOCAL_ALT[-1] not in ["/", "\\"]:
        LOCAL_ALT += getSlash(LOCAL_ALT)

    XBMC_PATH = args.prefix
    if XBMC_PATH and XBMC_PATH[-1] not in ["/", "\\"]:
        XBMC_PATH += getSlash(XBMC_PATH)
...
def main(args):
    ...
    if args.verbose:
        showConfig(args, download_items, season_items, episode_items)
    ...
    for media in data:
        if "movieid" in media:
            ...
            printerr(
                "FATAL: Unsupported input data - movie, sets and tvshow data for now!"
            )
            sys.exit(1)
        ...
        if mediatype == "tvshow":
            ...
            if args.output and workitem["items"]:
