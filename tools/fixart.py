...
def init():
    ...
    parser = argparse.ArgumentParser(
        description='Fix artwork URLs, replacing "--from url" with "--to url". \
...
def processitem(mediatype, media, urlfrom, urlto):
    item = {
        "type": mediatype,
        "libraryid": media["%sid" % mediatype],
        "title": media["label"],
        "items": {},
    }

    for art in media.get("art", []):
        if mediatype in ["tvshow", "season", "episode"] and art.startswith("tvshow."):
            continue
